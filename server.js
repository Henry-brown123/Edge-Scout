...
// TEMPORARY DIAGNOSTIC — pooled tier-level calibration + EV-ROI tiers +
// shrinkage + composite readiness ranking. Read-only, zero Odds API calls
// (reads backfill-historical.json + closing-odds.json, both already on disk).
// To be removed once docs/tier-calibration-analysis.md is written.
app.get('/api/debug/tier-calibration', (_req, res) => {
  ...
  // 5pp-wide tiers, 35% to 80% — see docs/tier-calibration-analysis.md for why
  // this width/range was chosen relative to confidence-ceiling-diagnostic.md's
  // 10pp bins.
  const EDGES = [0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80];
  function tierOf(p) {
    if (p < EDGES[0]) return '<35%';
    for (let i = 0; i < EDGES.length - 1; i++) {
      if (p >= EDGES[i] && p < EDGES[i + 1]) return `${Math.round(EDGES[i]*100)}-${Math.round(EDGES[i+1]*100)}%`;
    }
    return '80%+';
  }
  const TIER_LABELS = ['<35%', ...EDGES.slice(0, -1).map((e, i) => `${Math.round(e*100)}-${Math.round(EDGES[i+1]*100)}%`), '80%+'];

  // ── Pass 1: score every fixture with the live pipeline (GBDT + bias correction) ──
  const scored = [];
  for (const rec of scoredRecords) {
    ...
    scored.push({
      fixtureId: rec.fixtureId, leagueId, date: rec.date,
      league: LEAGUE_CONFIG[leagueId]?.name || `League ${leagueId}`,
      modelProb, correct: rec.actualOutcome === topOutcome,
      tier: tierOf(modelProb),
    });
  }
  // Only the 12 real LEAGUE_CONFIG leagues are in scope — stray IDs (old/misc
  // competitions not in LEAGUE_CONFIG) are excluded, same pattern as prior audits.
  const inScope = scored.filter(s => LEAGUE_CONFIG[s.leagueId]);

  // ── Phase 1.1/1.2: pooled tier table ──
  const pooledTiers = TIER_LABELS.map(label => {
    const rows = inScope.filter(s => s.tier === label);
    const n = rows.length;
  ...
  // ── Phase 1.3: per-league-within-tier, raw + shrunk ──
  const leagueIds = Object.keys(LEAGUE_CONFIG).map(Number);
  const tierLeagueRaw = {}; // tier -> [{leagueId, league, n, value(hitRate)}]
  for (const label of TIER_LABELS) {
    const pooled = pooledTiers.find(t => t.tier === label);
    const cells = [];
    for (const lid of leagueIds) {
      const rows = inScope.filter(s => s.tier === label && s.leagueId === lid);
      const n = rows.length;
      if (n === 0) continue;
      const hitRate = rows.filter(r => r.correct).length / n;
      cells.push({ id: LEAGUE_CONFIG[lid].name, leagueId: lid, n, value: +hitRate.toFixed(4), deviation: pooled ? +(hitRate - pooled.hitRate).toFixed(4) : null });
    }
    tierLeagueRaw[label] = cells;
  }
  const tierLeagueShrunk = {};
  for (const label of TIER_LABELS) {
    const cells = tierLeagueRaw[label];
    tierLeagueShrunk[label] = cells.length > 0 ? empiricalBayesShrink(cells, varianceForHitRate) : [];
  }

  // ── Phase 1.4 + 2b: EV-backtest ROI, same tiers, posEdge>=5%, matched to Pinnacle ──
  // Mirrors runEvCalibration()'s matching/edge logic exactly, but buckets by
  // model-probability tier instead of edge band, and keeps per-bet returns so
  // sample variance can feed the shrinkage function below.
  const matched = [];
  for (const rec of scoredRecords) {
    ...
    if (edge < 0.05) continue; // posEdge only, matches runEvCalibration's ROI population
    const split = VALIDATED_SPLITS[leagueId];
    if (split && new Date(rec.date) < new Date(split.testFrom)) continue; // never report train-tainted ROI
    const won = rec.actualOutcome === topOutcome;
    matched.push({ leagueId, league: LEAGUE_CONFIG[leagueId]?.name || `League ${leagueId}`, tier: tierOf(modelProb), returnPct: won ? (odds - 1) : -1 });
  }
  const roiTiers = TIER_LABELS.map(label => {
    const rows = matched.filter(m => m.tier === label);
    const n = rows.length;
    if (n === 0) return { tier: label, n: 0 };
    const roi = rows.reduce((s, r) => s + r.returnPct, 0) / n;
    return { tier: label, n, roi: +roi.toFixed(4) };
  });

  // ── Phase 2a: shrink league-level EV-ROI (reuses runEvCalibration's own byLeague figures + n/posEdgeN, adds variance for shrinkage input) ──
  const evCal = runEvCalibration();
  const roiCells = evCal.byLeague.map(l => {
    const rows = matched.filter(m => m.leagueId === l.leagueId);
    const n = rows.length; // should equal l.posEdgeN
    let sampleVariance = null;
    if (n > 1) {
      const mean = rows.reduce((s, r) => s + r.returnPct, 0) / n;
      sampleVariance = rows.reduce((s, r) => s + (r.returnPct - mean) ** 2, 0) / (n - 1);
    }
    return { id: l.league, leagueId: l.leagueId, n, value: l.roi ?? 0, sampleVariance, calibrationReliable: l.calibrationReliable, calibrationStatus: l.calibrationStatus };
  }).filter(c => c.n > 0);
  const roiShrunk = roiCells.length > 0
    ? empiricalBayesShrink(roiCells, c => (typeof c.sampleVariance === 'number' ? c.sampleVariance : varianceForHitRate({ value: 0.5 })))
    : [];

  // ── Phase 3: composite readiness score, league-level ──
  // calibScore: weighted-mean |shrunk calibration deviation| across tiers for
  // that league (weighted by cell n), lower = better, then min-max normalised
  // across leagues to 0-1 (1 = best calibration).
  const calibByLeague = {};
  for (const lid of leagueIds) {
    let wSum = 0, nSum = 0;
    for (const label of TIER_LABELS) {
      const cell = (tierLeagueShrunk[label] || []).find(c => c.leagueId === lid);
      if (!cell) continue;
      wSum += Math.abs(cell.delta !== null ? (cell.pooledMean + cell.weight * (cell.value - cell.pooledMean)) - pooledTiers.find(t => t.tier === label).hitRate : 0) * cell.n;
      nSum += cell.n;
    }
    if (nSum > 0) calibByLeague[lid] = { weightedAbsShrunkDeviation: wSum / nSum, n: nSum };
  }
  const calibVals = Object.values(calibByLeague).map(v => v.weightedAbsShrunkDeviation);
  const calibMin = Math.min(...calibVals), calibMax = Math.max(...calibVals);
  const roiVals = roiShrunk.map(c => c.shrunk);
  const roiMin = Math.min(...roiVals), roiMax = Math.max(...roiVals);
  const bets = getBets();
  const composite = leagueIds.filter(lid => calibByLeague[lid] && roiShrunk.find(c => c.leagueId === lid)).map(lid => {
    const calibDev = calibByLeague[lid].weightedAbsShrunkDeviation;
    const calibScore = calibMax > calibMin ? 1 - (calibDev - calibMin) / (calibMax - calibMin) : 0.5;
    const roiCell = roiShrunk.find(c => c.leagueId === lid);
    const roiScore = roiMax > roiMin ? (roiCell.shrunk - roiMin) / (roiMax - roiMin) : 0.5;
    const paperCount = bets.filter(b => parseInt(b.leagueId, 10) === lid && b.mode === 'paper').length;
    const tradeScore = Math.min(paperCount, 20) / 20;
    const W = { calib: 0.5, roi: 0.35, trade: 0.15 };
    const compositeScore = W.calib * calibScore + W.roi * roiScore + W.trade * tradeScore;
    return {
      league: LEAGUE_CONFIG[lid].name, leagueId: lid,
      calibWeightedAbsDeviation: +calibDev.toFixed(4), calibScore: +calibScore.toFixed(3),
      shrunkRoi: roiCell.shrunk, roiRaw: roiCell.value, roiN: roiCell.n, roiScore: +roiScore.toFixed(3),
      paperTradeCount: paperCount, tradeScore: +tradeScore.toFixed(3),
      compositeScore: +compositeScore.toFixed(4),
      calibrationReliable: roiCell.calibrationReliable, calibrationStatus: roiCell.calibrationStatus,
    };
  }).sort((a, b) => b.compositeScore - a.compositeScore);

  // Tier-level viability: flag (league,tier) cells with n >= 200 as sample
  // size genuinely supporting tier-level granularity (binomial SE ~<=3.5pp at
  // p~0.5, n=200) — everything else falls back to league-level only.
  const TIER_VIABLE_N = 200;
  const tierViable = [];
  for (const label of TIER_LABELS) {
    for (const cell of (tierLeagueRaw[label] || [])) {
      if (cell.n >= TIER_VIABLE_N) tierViable.push({ tier: label, league: cell.id, leagueId: cell.leagueId, n: cell.n });
    }
  }

  res.json({
    scope: { totalScoredRecords: scoredRecords.length, inScopeFixtures: inScope.length, tierEdges: EDGES, tierLabels: TIER_LABELS },
    pooledTiers,
    tierLeagueRaw,
    tierLeagueShrunk,
    roiTiers,
    roiLeagueShrink: { raw: roiCells, shrunk: roiShrunk },
    compositeReadiness: composite,
    tierViableCells: tierViable,
    weightsUsed: { calib: 0.5, roi: 0.35, trade: 0.15, tierViableMinN: TIER_VIABLE_N },
  });
});
