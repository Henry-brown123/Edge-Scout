...
// TEMPORARY DIAGNOSTIC — follow-up to docs/tier-calibration-analysis.md:
// (A) test-only tier calibration recheck for the 4 validated leagues,
// (B) reliability-aware second composite ranking. Read-only, zero Odds API
// calls. To be removed once the doc update is written.
app.get('/api/debug/tier-calibration-v2', (_req, res) => {
  ...
  // ── Score every fixture with the live pipeline (GBDT + bias correction) ──
  const scored = [];
  for (const rec of scoredRecords) {
    ...
    scored.push({
      leagueId, date: rec.date, league: LEAGUE_CONFIG[leagueId]?.name || `League ${leagueId}`,
      modelProb, correct: rec.actualOutcome === topOutcome, tier: tierOf(modelProb),
    });
  }
  ...
  function tierTable(rows) {
    return TIER_LABELS.map(label => {
      const cell = rows.filter(r => r.tier === label);
      const n = cell.length;
      if (n === 0) return { tier: label, n: 0 };
      const meanPredicted = cell.reduce((s, r) => s + r.modelProb, 0) / n;
      const hitRate = cell.filter(r => r.correct).length / n;
      return {
        tier: label, n, meanPredicted: +meanPredicted.toFixed(4), hitRate: +hitRate.toFixed(4),
        calibrationError: +(meanPredicted - hitRate).toFixed(4), smallSample: n < SMALL_SAMPLE_N,
      };
    });
  }

  // ── Part A: mixed vs test-only tier calibration, for the 4 validated leagues ──
  const validatedComparison = {};
  for (const lid of Object.keys(VALIDATED_SPLITS).map(Number)) {
    const split = VALIDATED_SPLITS[lid];
    const leagueRows = inScope.filter(s => s.leagueId === lid);
    const testOnlyRows = leagueRows.filter(s => new Date(s.date) >= new Date(split.testFrom));
    validatedComparison[LEAGUE_CONFIG[lid].name] = {
      leagueId: lid,
      testFrom: split.testFrom,
      mixedN: leagueRows.length,
      testOnlyN: testOnlyRows.length,
      mixed: tierTable(leagueRows),
      testOnly: tierTable(testOnlyRows),
    };
  }

  // ── Part B: reuse the same composite-readiness computation as before ──
  const leagueIds = Object.keys(LEAGUE_CONFIG).map(Number);
  const pooledTiers = tierTable(inScope);
  const tierLeagueRaw = {};
  ...
  for (const label of TIER_LABELS) {
    tierLeagueShrunk[label] = tierLeagueRaw[label].length > 0 ? empiricalBayesShrink(tierLeagueRaw[label], varianceForHitRate) : [];
  }
  ...
  for (const rec of scoredRecords) {
    ...
    if (split && new Date(rec.date) < new Date(split.testFrom)) continue;
    const won = rec.actualOutcome === topOutcome;
    matched.push({ leagueId, returnPct: won ? (odds - 1) : -1 });
  }
  ...
  for (const lid of leagueIds) {
    ...
    for (const label of TIER_LABELS) {
      ...
      if (!cell) continue;
      wSum += Math.abs((cell.pooledMean + cell.weight * (cell.value - cell.pooledMean)) - pooledTiers.find(t => t.tier === label).hitRate) * cell.n;
      nSum += cell.n;
    }
  ...
  const composite = leagueIds.filter(lid => calibByLeague[lid] && roiShrunk.find(c => c.leagueId === lid)).map(lid => {
    ...
    return {
      ...
      calibScore: +calibScore.toFixed(3), roiScore: +roiScore.toFixed(3),
      tradeScore: +tradeScore.toFixed(3), shrunkRoi: roiCell.shrunk, paperTradeCount: paperCount,
  ...
  // Reliability-aware second table: simple filter (option a), not a down-weight —
  // same compositeScore values as the raw table, just restricted to leagues
  // whose calibration is actually validated, so ranks are comparable across leagues
  // that earned them the same way.
  const reliabilityFiltered = composite.filter(c => c.calibrationReliable === true);

  res.json({
    scope: { totalScoredRecords: scoredRecords.length, inScopeFixtures: inScope.length },
    validatedComparison,
    compositeRaw: composite,
    compositeReliabilityFiltered: reliabilityFiltered,
  });
});
