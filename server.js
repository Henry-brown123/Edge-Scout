...
// TEMPORARY diagnostic endpoint — remove after La Liga ROI regression investigation.
// Recomputes the EV-calibration positive-edge set for one league, with optional
// config overrides (to reproduce a "before" scenario) and a home/away/draw split.
app.get('/api/debug/ev-league-detail', (req, res) => {
  try {
    ...
    const overrideAwayRate = req.query.avgAwayWinRate    != null ? parseFloat(req.query.avgAwayWinRate)    : null;
    const overrideDrawRate = req.query.avgDrawRate       != null ? parseFloat(req.query.avgDrawRate)       : null;
    ...
    for (const rec of scoredRecords) {
      ...
      if (overrideHomeAdv  != null) leagueConfig.homeAdvBaseWeight = overrideHomeAdv;
      if (overrideAwayRate != null) leagueConfig.avgAwayWinRate    = overrideAwayRate;
      if (overrideDrawRate != null) leagueConfig.avgDrawRate       = overrideDrawRate;
    ...
    const bands = BANDS.map(b => {
      const inBand = matched.filter(f => f.edge >= b.min && f.edge < b.max);
    ...
    res.json({
      leagueId,
      configUsed: { homeAdvBaseWeight: overrideHomeAdv ?? LEAGUE_CONFIG[leagueId]?.homeAdvBaseWeight, avgAwayWinRate: overrideAwayRate ?? LEAGUE_CONFIG[leagueId]?.avgAwayWinRate, avgDrawRate: overrideDrawRate ?? LEAGUE_CONFIG[leagueId]?.avgDrawRate },
