...
// ─── TEMP DIAGNOSTIC (2026-09-01, round 7): tournament-side coverage gap check ───
// The audit's lowConfidence/dataConfMin overlay checks were scoped to
// DOMESTIC_LEAGUE_IDS_FOR_BLEND only — tournament was never separately tested for
// either gate, and the PL edge-cap check excluded tournament from both its groups.
// Carabao Cup (48) is a genuine edge case: TOURNAMENT_LEAGUE_IDS for betting-tier
// purposes, but classifyFixture(48) returns 'club_domestic' for SCORING context
// (uses CONTEXT_CONFIG.club_domestic's thresholds live) — it fell through both the
// domestic filter (not in DOMESTIC_LEAGUE_IDS_FOR_BLEND) and a context-based
// club_european/international filter in every prior diagnostic. This test uses
// TOURNAMENT_LEAGUE_IDS (the betting-tier definition) consistently, matching how
// getCalFactorForLeague already treats Carabao Cup, and picks each fixture's
// lowConfidence/dataConfMin thresholds from ITS OWN actual scoring context
// (club_domestic for Carabao Cup, club_european or international for the rest) —
// i.e. tests exactly what live scoring actually computes for each league, not one
// assumed context for all five. Remove once answered.
async function computeTournamentGateCheck() {
  const { classifyFixture, applyLeagueBiasCorrection, LEAGUE_CONFIG, marginStrippedImplied,
    TOURNAMENT_LEAGUE_IDS } = require('./scoring');
  const historical    = readHistoricalCached() || {};
  ...
  for (const rec of scoredRecords) {
    ...
    if (!TOURNAMENT_LEAGUE_IDS.has(leagueId)) continue;
    ...
    const maxModelBookGap = Math.max(
      Math.abs(probs.home - stripped.home), Math.abs(probs.draw - stripped.draw), Math.abs(probs.away - stripped.away),
    );
    ...
    const minFormCount  = Math.min(homeFormCount, awayFormCount);
    const tierThreshold = minFormCount < 20 ? 0.08 : minFormCount < 35 ? 0.12 : 0.18;
    const wouldDropLowConf   = maxModelBookGap > gapThreshold || maxModelBookGap > tierThreshold;
    const dataMin            = cfg.dataConfMin ?? 0.3;
    const wouldDropDataMin   = homeDataConf < dataMin && awayDataConf < dataMin;

    const won = rec.actualOutcome === topOutcome;
    rows.push({ leagueId, date: rec.date, pinnacleOdds, won, edge, wouldDropLowConf, wouldDropDataMin });
  }
  ...
  const byLeagueCount = {};
  for (const r of rows) byLeagueCount[r.leagueId] = (byLeagueCount[r.leagueId] || 0) + 1;

  return {
    composition: byLeagueCount,
    lowConfidence: {
      wouldSurvive: summarise(rows.filter(r => !r.wouldDropLowConf)),
      wouldBeDropped: summarise(rows.filter(r => r.wouldDropLowConf)),
    },
    dataConfMin: {
      wouldSurvive: summarise(rows.filter(r => !r.wouldDropDataMin)),
      wouldBeDropped: summarise(rows.filter(r => r.wouldDropDataMin)),
    },
    edgeCap20pct: {
      below20: summarise(rows.filter(r => r.edge < 0.20)),
      at20OrAbove: summarise(rows.filter(r => r.edge >= 0.20)),
    },
  };
}
app.get('/api/admin/tournament-gate-check', async (_req, res) => {
  try { res.json(await computeTournamentGateCheck()); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
