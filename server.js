...
// TEMP diagnostic — disaggregates Addendum 14's continuous-ROI population into
// cleared-threshold vs blocked, per tier and per league. Reuses the EXACT same
// population/model/methodology as Addendum 14's evidence-table endpoint (no new
// odds/API calls, no re-fit, no new "look" — this is the same single test-set
// look, split differently). See docs/tier-calibration-analysis.md Addendum 14
// extension for the write-up this feeds.
app.get('/api/debug/threshold-split', (_req, res) => {
  try {
    const HOLDOUT_START = '2024-08-07T00:00:00.000Z';
    const EDGE_THRESHOLD = 0.05; // same "posEdge >= 5%" convention as every prior addendum
    const historical = readJSON('backfill-historical.json') || {};
    ...
    // ── Score every in-scope fixture through the PROXY model — identical to Addendum 14 ──
    const scored = [];
    ...
    const holdoutMatched = scored.filter(r => r.date >= HOLDOUT_START && validatedIds.includes(r.leagueId) && r.edge != null);

    // ── Stats helpers ──
    function statsFor(pop) {
      const n = pop.length;
      if (!n) return { n: 0, roi: null, ci: null };
    ...
    function splitPop(pop) {
      return {
        cleared:  pop.filter(r => r.edge >= EDGE_THRESHOLD),
        blocked:  pop.filter(r => r.edge >= 0 && r.edge < EDGE_THRESHOLD),
        negative: pop.filter(r => r.edge < 0),
      };
    }

    // ── Pooled, per tier ──
    const pooledByTier = CALIB_TIER_LABELS.map(label => {
      const t = TIERS.find(x => x.label === label);
      const inTier = holdoutMatched.filter(r => r.modelProb >= t.lo && r.modelProb < t.hi);
      const { cleared, blocked, negative } = splitPop(inTier);
      return {
        tier: label,
        totalN: inTier.length,
        cleared: statsFor(cleared),
        blocked: statsFor(blocked),
        negative: statsFor(negative),
      };
    });

    // ── Per league, per tier (only where any group has n>0) ──
    const byLeague = {};
    for (const lid of validatedIds) {
      byLeague[lid] = CALIB_TIER_LABELS.map(label => {
        const t = TIERS.find(x => x.label === label);
        const inTier = holdoutMatched.filter(r => r.leagueId === lid && r.modelProb >= t.lo && r.modelProb < t.hi);
        const { cleared, blocked, negative } = splitPop(inTier);
        return { tier: label, totalN: inTier.length, cleared: statsFor(cleared), blocked: statsFor(blocked), negative: statsFor(negative) };
      });
    }

    // ── Edge-size distribution WITHIN the blocked group (pooled) ──
    const BLOCKED_SUBBANDS = [
      { label: '0-1%', lo: 0,    hi: 0.01 },
      { label: '1-2%', lo: 0.01, hi: 0.02 },
      { label: '2-3%', lo: 0.02, hi: 0.03 },
      { label: '3-4%', lo: 0.03, hi: 0.04 },
      { label: '4-5%', lo: 0.04, hi: 0.05 },
    ];
    const allBlockedPooled = holdoutMatched.filter(r => r.edge >= 0 && r.edge < EDGE_THRESHOLD);
    const blockedEdgeDistribution = BLOCKED_SUBBANDS.map(b => {
      const inBand = allBlockedPooled.filter(r => r.edge >= b.lo && r.edge < b.hi);
      return { band: b.label, ...statsFor(inBand) };
    });
    const blockedEdgeSummary = {
      n: allBlockedPooled.length,
      meanEdgePct: allBlockedPooled.length ? +((allBlockedPooled.reduce((s, r) => s + r.edge, 0) / allBlockedPooled.length) * 100).toFixed(2) : null,
      medianEdgePct: (() => {
        if (!allBlockedPooled.length) return null;
        const sorted = allBlockedPooled.map(r => r.edge).sort((a, b) => a - b);
        const mid = Math.floor(sorted.length / 2);
        return +((sorted.length % 2 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2) * 100).toFixed(2);
      })(),
    };
    ...
    res.json({
      ...
      liveSuccessThreshold: (readJSON('settings.json') || {}).successThreshold ?? 40,
      totalMatchedHoldoutPop: holdoutMatched.length,
      pooledByTier,
      byLeague,
      blockedEdgeDistribution,
      blockedEdgeSummary,
    });
  } catch (e) {
