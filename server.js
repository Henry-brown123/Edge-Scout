...
// ═══ TEMP — standings-proxy candidate test (design brief Part 3) ════════════
// Read-only, train-only in spirit: tests whether last-season final standing or
// market-implied strength (closing-odds proxy, opening-line data isn't captured
// separately in this codebase) carry real discriminating power for early-season
// fixtures, versus the current in-season rolling rank and flat neutral. No model
// change, no training — just correlation reads against already-stored data.
// Removed once Part 3's recommendation is delivered.
const PHASE3_LEAGUES = [39, 140, 135, 78, 61]; // deep, continuous history — PL/La Liga/Serie A/Bundesliga/Ligue1

app.get('/api/diagnostics/phase3-standings-proxy', (_req, res) => {
  const hist = readJSON('backfill-historical.json');
  const fixtures = (hist?.fixtures || []).filter(f =>
    PHASE3_LEAGUES.includes(f.league?.id) &&
    f.fixture?.status?.short === 'FT' &&
    Number.isFinite(Number(f.goals?.home)) && Number.isFinite(Number(f.goals?.away)));
  const closingOdds = readJSON('closing-odds.json') || {};

  // Group by league+season, sorted chronologically.
  const bySeason = {};
  ...
  for (const key of Object.keys(bySeason)) {
    bySeason[key].sort((a, b) => new Date(a.fixture.date) - new Date(b.fixture.date));
  }

  // End-of-season rank per (leagueId, season, teamId) — last rolling snapshot.
  const seasonEndRank = {}; // `${leagueId}_${season}_${teamId}` -> {rank, leagueSize}

  // Per-fixture computed rows, built while replaying each season chronologically
  // (rolling in-season rank/games-played, no look-ahead — same discipline as
  ...
  for (const key of orderedSeasonKeys) {
    ...
    for (const f of seasonFixtures) {
      const hid = f.teams?.home?.id, aid = f.teams?.away?.id, fid = f.fixture?.id;
      if (!hid || !aid || !fid) continue;

      const teamSet = new Set([...Object.keys(pts), String(hid), String(aid)]);
      const teamList = [...teamSet].sort((a, b) => (pts[b] || 0) - (pts[a] || 0));
      const leagueSize = teamList.length;
      const homeRank = teamList.indexOf(String(hid)) + 1;
      const awayRank = teamList.indexOf(String(aid)) + 1;
      const homeGamesPlayed = played[String(hid)] || 0;
      const awayGamesPlayed = played[String(aid)] || 0;

      const lastSeasonKeyHome = `${leagueId}_${season - 1}_${hid}`;
      const lastSeasonKeyAway = `${leagueId}_${season - 1}_${aid}`;
      const lastSeasonHome = seasonEndRank[lastSeasonKeyHome] || null;
      const lastSeasonAway = seasonEndRank[lastSeasonKeyAway] || null;

      const co = closingOdds[String(fid)] || closingOdds[fid];
      let marketHomeProb = null;
      if (co?.homeOdds != null && co?.drawOdds != null && co?.awayOdds != null && co.homeOdds > 0 && co.drawOdds > 0 && co.awayOdds > 0) {
        const invH = 1 / co.homeOdds, invD = 1 / co.drawOdds, invA = 1 / co.awayOdds;
        const overround = invH + invD + invA;
        marketHomeProb = invH / overround; // devigged
      }

      const hg = Number(f.goals.home), ag = Number(f.goals.away);
      const actualOutcome = hg > ag ? 'home' : (hg < ag ? 'away' : 'draw');

      rows.push({
        leagueId, season, fixtureId: fid,
        homeGamesPlayed, awayGamesPlayed,
        currentHomeProxy: Math.round(((leagueSize - homeRank + 1) / leagueSize) * 100),
        currentAwayProxy: Math.round(((leagueSize - awayRank + 1) / leagueSize) * 100),
        lastSeasonHomeProxy: lastSeasonHome ? Math.round(((lastSeasonHome.leagueSize - lastSeasonHome.rank + 1) / lastSeasonHome.leagueSize) * 100) : null,
        lastSeasonAwayProxy: lastSeasonAway ? Math.round(((lastSeasonAway.leagueSize - lastSeasonAway.rank + 1) / lastSeasonAway.leagueSize) * 100) : null,
        marketHomeProb,
        actualOutcome,
      });
      ...
      played[String(hid)] = homeGamesPlayed + 1;
      played[String(aid)] = awayGamesPlayed + 1;

      seasonEndRank[`${leagueId}_${season}_${hid}`] = { rank: homeRank, leagueSize };
      seasonEndRank[`${leagueId}_${season}_${aid}`] = { rank: awayRank, leagueSize };
    }
  }

  // Pearson correlation between a proxy differential and actual outcome
  // (home=1, draw=0, away=-1) — a single, comparable number per candidate.
  function pearson(xs, ys) {
    ...
    return denom > 0 ? +(num / denom).toFixed(4) : null;
  }
  const outcomeCode = o => o === 'home' ? 1 : (o === 'away' ? -1 : 0);

  function analyzeBucket(bucketRows, label) {
    const results = {};
    const cands = {
      current:    r => (r.currentHomeProxy != null && r.currentAwayProxy != null) ? r.currentHomeProxy - r.currentAwayProxy : null,
      lastSeason: r => (r.lastSeasonHomeProxy != null && r.lastSeasonAwayProxy != null) ? r.lastSeasonHomeProxy - r.lastSeasonAwayProxy : null,
      market:     r => r.marketHomeProb != null ? (r.marketHomeProb * 100 - 50) : null, // centered
    };
    for (const [name, fn] of Object.entries(cands)) {
      ...
      for (const r of bucketRows) {
        const x = fn(r);
        if (x == null) continue;
        xs.push(x); ys.push(outcomeCode(r.actualOutcome));
      }
      if (xs.length < 20) { results[name] = { n: xs.length, note: 'too few to read' }; continue; }
      // Tercile split for an intuitive home-win-rate gap.
      const sorted = xs.map((x, i) => [x, ys[i]]).sort((a, b) => a[0] - b[0]);
      const third = Math.floor(sorted.length / 3);
      const low  = sorted.slice(0, third);
      const high = sorted.slice(-third);
      const homeWinRate = arr => arr.filter(([, y]) => y === 1).length / arr.length;
      results[name] = {
        n: xs.length,
        pearsonR: pearson(xs, ys),
        lowTercileHomeWinRate: +homeWinRate(low).toFixed(3),
        highTercileHomeWinRate: +homeWinRate(high).toFixed(3),
      };
    }
    return { bucket: label, n: bucketRows.length, candidates: results };
  }

  const buckets = [0, 1, 2, 3, 4, 5].map(g => {
    const bucketRows = rows.filter(r => Math.min(r.homeGamesPlayed, r.awayGamesPlayed) === g);
    return analyzeBucket(bucketRows, `${g}_games_played_min`);
  });
  const preThreshold  = analyzeBucket(rows.filter(r => Math.min(r.homeGamesPlayed, r.awayGamesPlayed) < 3), 'pre_threshold_lt3');
  const postThreshold = analyzeBucket(rows.filter(r => Math.min(r.homeGamesPlayed, r.awayGamesPlayed) >= 3 && Math.min(r.homeGamesPlayed, r.awayGamesPlayed) < 10), 'post_threshold_3to9');

  res.json({
    totalFixturesConsidered: rows.length,
    note: 'marketHomeProb is closing-odds implied probability (devigged) for THIS specific fixture, not a general pre-season team-strength baseline — opening-line data is not captured separately in this codebase. lastSeasonProxy unavailable for promoted/newly-tracked teams (no prior-season rank in this league).',
    byExactGamesPlayed: buckets,
    preThreshold, postThreshold,
  });
});
