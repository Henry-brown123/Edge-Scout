...
// TEMP DIAGNOSTIC — remove after use. User question: if you pool every genuine
// domestic league (DOMESTIC_LEAGUE_IDS_FOR_BLEND — no tournaments/cups) and
// restrict to modelProb >= 45% or >= 50%, does the aggregate ROI turn
// positive, given the pooled figure across everything is roughly flat/
// negative? Purely a re-slice of already-computed per-fixture data (same
// modelProb/edge/pinnacleOdds/won values every per-league figure already
// comes from) at a different threshold and grouping — no new fitted
// parameter, not a new statistical test, informational only, not a basis for
// any settings/green-flag change on its own. Each league's own test-only
// population definition is honored exactly as elsewhere (VALIDATED_SPLITS
// testFrom for the original 9, rule-12 date-split cutoffs for League One/Two/
// Championship) so this doesn't mix in-sample data into the pool.
app.get('/api/admin/diag-domestic-pooled-by-prob-floor', async (_req, res) => {
  try {
    ...
    const pool = matched.filter(f => {
      ...
      if (dsCutoff) return new Date(f.date) < new Date(dsCutoff); // banked pre-cutoff population
      const split = VALIDATED_SPLITS[lid];
      if (!split) return true;
      return new Date(f.date) >= new Date(split.testFrom); // held-out test-set only
    });
    ...
    const FLOORS = [
      { label: 'no floor (everything)', min: 0 },
      { label: '>=45%', min: 0.45 },
      { label: '>=50%', min: 0.50 },
    ];

    const byFloor = FLOORS.map(fl => {
      const posEdge = pool.filter(f => f.edge >= 0.05 && f.modelProb >= fl.min);
      const returns = posEdge.map(f => f.won ? (f.pinnacleOdds - 1) : -1);
      const ci = pooledCi(returns);
      // Per-league contribution at this floor, for sanity-checking the pooled figure
      // isn't dominated by one league's idiosyncrasy.
      const byLeague = {};
      for (const f of posEdge) {
        const lid = parseInt(f.leagueId, 10);
        (byLeague[lid] = byLeague[lid] || []).push(f);
      }
      const leagueBreakdown = Object.entries(byLeague).map(([lid, fs]) => {
        const r = fs.map(f => f.won ? (f.pinnacleOdds - 1) : -1);
        const c = pooledCi(r);
        return { leagueId: parseInt(lid, 10), n: c.n, roi: c.roi };
      }).sort((a, b) => b.n - a.n);
      return { floor: fl.label, ...ci, leagueBreakdown };
    });

    res.json({
      note: 'TEMP diagnostic — pools every genuine domestic league (no tournaments/cups), posEdge>=5% fixtures only, at three modelProb floors: no floor, >=45%, >=50%. Each league restricted to its own test-only/pre-cutoff population (same populations every other addendum in this doc uses — see VALIDATED_SPLITS and the rule-12 date-split cutoffs). Informational re-slice only — not a new statistical test, not a basis for a settings change on its own. Delete this endpoint once read.',
