...
// ─── TEMP DIAGNOSTIC (2026-09-04, Addendum 38) — remove after use ──────────────
// Threshold search for the 18%/45% rule on the Championship/League One/League Two
// population only, under calibration-rules.md discipline: the (edge, prob) grid is
// searched on TRAIN only (fixtures before 2024-09-16 — the start of the concurrent
// window the original rule was selected on), the winning combination is chosen by a
// rule fixed here in code, and the endpoint reveals TEST figures (2024-09-16 → each
// league's rule-12 cutoff) for exactly two combinations: the train-selected one and
// the incumbent 18/45. The test grid is never computed, so it cannot be peeked at.
// These three leagues were excluded wholesale from the live model's 2026-08-08
// training (League One/Two under rule 10 at the time; Championship not yet ingested),
// so every fixture here is tree-clean regardless of the date-only rule-16 tag.
app.get('/api/admin/diag-rule12-threshold-search', async (_req, res) => {
  try {
    const { LEAGUE_CONFIG } = require('./scoring');
    ...
    const stats = arr => {
      const n = arr.length;
      if (!n) return { n: 0, roi: null, ci95: null, absReturn: 0 };
      ...
      const dates = arr.map(f => f.date).sort();
      return { n, wins: arr.filter(f => f.won).length, roi: +(mean * 100).toFixed(2), ci95: [+((mean - half) * 100).toFixed(2), +((mean + half) * 100).toFixed(2)],
        absReturn: +(rets.reduce((s, v) => s + v, 0)).toFixed(1), avgOdds: +(arr.reduce((s, f) => s + f.pinnacleOdds, 0) / n).toFixed(2), from: dates[0]?.slice(0, 10), to: dates[dates.length - 1]?.slice(0, 10) };
    };
    ...
    const EDGE = [0.10, 0.12, 0.14, 0.15, 0.16, 0.18, 0.20, 0.22, 0.24];
    const PROB = [0, 0.40, 0.45, 0.50];
    const trainBlocks = blocksOf(train);
    const grid = [];
    for (const e of EDGE) for (const p of PROB) {
      const cell = train.filter(f => passes(f, e, p));
      const perBlock = trainBlocks.map(b => stats(b.filter(f => passes(f, e, p))));
      const positiveBlocks = perBlock.filter(b => b.n >= 20 && b.roi > 0).length;
      grid.push({ edge: e, prob: p, ...stats(cell), perBlock: perBlock.map(b => ({ n: b.n, roi: b.roi })), positiveBlocks, robust: positiveBlocks === 4 });
    }
    // Pre-registered selection rule: robust (all 4 train blocks positive at n>=20)
    // and train n>=100; maximise absolute return; ties → higher edge floor.
    const eligible = grid.filter(g => g.robust && g.n >= 100);
    const pick = (eligible.length ? eligible : grid.filter(g => g.positiveBlocks >= 3 && g.n >= 100))
      .sort((a, b) => (b.absReturn - a.absReturn) || (b.edge - a.edge))[0] || null;

    const testLook = (e, p, label) => {
      const cell = test.filter(f => passes(f, e, p));
      return { label, edge: e, prob: p, pooled: stats(cell), byLeague: group(cell, name),
        perBlock: blocksOf(test).map(b => { const c = b.filter(f => passes(f, e, p)); const st = stats(c); return { range: b.length ? `${b[0].date.slice(0, 10)} to ${b[b.length - 1].date.slice(0, 10)}` : 'empty', n: st.n, roi: st.roi, ci95: st.ci95 }; }) };
    };
    const incumbent = testLook(PAPER_MONEY_EDGE_MIN, PAPER_MONEY_PROB_MIN, 'incumbent 18/45');
    const selected = pick ? testLook(pick.edge, pick.prob, 'train-selected') : null;

    res.json({
      note: 'TEMP — Addendum 38. Train-only grid search on Championship/League One/League Two; single pre-registered test look. Remove after use.',
      scoringModel: getModelTreeBoundary(), calFactor: getCalFactorForLeague(getSettings(), 40),
      population: { leagues: ['Championship', 'League One', 'League Two'], preCutoffOnly: true, total: pop.length, train: { n: train.length, from: train[0]?.date.slice(0, 10), to: train[train.length - 1]?.date.slice(0, 10) }, test: { n: test.length, from: test[0]?.date.slice(0, 10), to: test[test.length - 1]?.date.slice(0, 10) },
        trainByLeague: group(train, name), testByLeague: group(test, name),
        preTreeBoundaryTagged: pop.filter(f => f.preTreeBoundary === true).length, treeCleanNote: 'all three leagues were excluded wholesale from the 2026-08-08 training run; the date-only rule-16 tag overstates exposure here' },
      selectionRule: 'robust (all 4 equal-count train blocks positive, each n>=20) AND train n>=100; maximise absolute return (sum of per-bet returns); ties → higher edge floor; fallback: >=3 positive blocks',
      trainGrid: grid.sort((a, b) => (b.robust - a.robust) || (b.absReturn - a.absReturn)),
      trainPick: pick ? { edge: pick.edge, prob: pick.prob, n: pick.n, roi: pick.roi, ci95: pick.ci95, absReturn: pick.absReturn, robust: pick.robust } : null,
      trainIncumbent: grid.find(g => g.edge === PAPER_MONEY_EDGE_MIN && g.prob === PAPER_MONEY_PROB_MIN),
      testLook: { selected, incumbent, sameCombination: !!pick && pick.edge === PAPER_MONEY_EDGE_MIN && pick.prob === PAPER_MONEY_PROB_MIN },
    });
  } catch (e) {
    res.status(500).json({ error: e.message, stack: e.stack });
  }
