...
// TEMPORARY diagnostic endpoint — check fixture-ID overlap between the population used
// by /api/league/diagnostic (base-rate tuning) and the population used by /api/ev-calibration
// (Pinnacle-matched ROI test) for a given league. Read-only, no API credits.
app.get('/api/debug/leakage-check', (req, res) => {
  try {
    ...
    // Population A: everything /api/league/diagnostic uses to compute "actual" rates
    // (has actualOutcome, matches league).
    const diagnosticIds = new Set();
    for (const rec of scoredRecords) {
      if (parseInt(rec.leagueId, 10) !== leagueId) continue;
      if (!rec.actualOutcome) continue;
      diagnosticIds.add(rec.fixtureId);
    }

    // Population B: everything /api/ev-calibration uses (same + requires Pinnacle match).
    const evMatched = [];
    for (const rec of scoredRecords) {
      if (parseInt(rec.leagueId, 10) !== leagueId) continue;
      ...
      evMatched.push({ fixtureId: rec.fixtureId, edge, won, pinnacleOdds, inDiagnosticPopulation: diagnosticIds.has(rec.fixtureId) });
    }

    function roi(arr) { if (!arr.length) return null; return arr.reduce((s, f) => s + (f.won ? (f.pinnacleOdds - 1) : -1), 0) / arr.length; }
    const posEdgeAll = evMatched.filter(f => f.edge >= 0.05);
    const overlapping = evMatched.filter(f => f.inDiagnosticPopulation);
    const holdout = evMatched.filter(f => !f.inDiagnosticPopulation);
    const posEdgeHoldout = holdout.filter(f => f.edge >= 0.05);

    res.json({
      leagueId,
      diagnosticPopulationSize: diagnosticIds.size,
      evCalibrationPopulationSize: evMatched.length,
      overlapCount: overlapping.length,
      overlapPct: evMatched.length ? +(overlapping.length / evMatched.length * 100).toFixed(2) : null,
      holdoutCount: holdout.length,
      fullSet: { posEdgeN: posEdgeAll.length, roi: roi(posEdgeAll) },
      holdoutOnly: { posEdgeN: posEdgeHoldout.length, roi: roi(posEdgeHoldout) },
    });
  } catch (e) {
