...
// model's raw output toward LEAGUE_CONFIG's target rates. This replays that real pipeline:
// GBDT predict() -> applyLeagueBiasCorrection() with an overridden avgAwayWinRate.
app.get('/api/debug/ev-bias-correction-detail', (req, res) => {
  try {
    const leagueId = parseInt(req.query.league, 10);
    const { classifyFixture, applyLeagueBiasCorrection, LEAGUE_CONFIG } = require('./scoring');
    const gbdt = require('./models/gbdt');
    const historical = readJSON('backfill-historical.json') || {};
    const scoredRecords = historical.scoredRecords || [];
    const optWeights = historical.optimisedWeights || {};
    const closingOdds = readJSON('closing-odds.json') || {};
    ...
    const leagueConfigOverride = {
      [leagueId]: {
        ...baseConfig,
        avgAwayWinRate: overrideAwayRate != null ? overrideAwayRate : baseConfig.avgAwayWinRate,
      },
    };
    ...
    for (const rec of scoredRecords) {
      ...
      const rawProbs = gbdt.predict(rec.homeFactors, rec.awayFactors, weights, context);
    ...
    res.json({
      leagueId,
      avgAwayWinRateUsed: leagueConfigOverride[leagueId].avgAwayWinRate,
