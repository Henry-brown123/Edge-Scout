...
(function seedStaticFiles() {
  ...
  if (!fs.existsSync(settingsDest)) {
    const defaults = { wowyActive: true,
      activeLeagues: ['1','39','140','78','135','61','2','179','88','94','3','848','48','41','42','40','136','141','79'], successThreshold: 40,
...
// Serie B (136), Segunda División (141), 2. Bundesliga (79) — added 2026-09-04
// as rule-10 holdouts from day one: their historical backfill is the one clean,
// never-trained-on population Phase 2's single backtest reads. Per rule 15 the
// holdout is temporary by design and its conversion is stated here up front:
// the moment each league's single disciplined look is computed and recorded in
// CALIBRATION_AUDIT, that league converts to a rule-12 date-split at a cutoff
// anchored to that backtest's own compute time (DATE_SPLIT_HOLDOUT_CUTOFFS here,
...
// tier 'real' = real-money pocket (bet locked at paper Kelly, converted by hand;
// shows orange until confirmed); tier 'paper' = paper-staked pocket (bet locked at
...
function assignPocket(leagueId, edge, modelProb, kickoffIso, standalone = null) {
  const lid = parseInt(leagueId, 10);
...
// League Two (213 of the 273 validated bets; the eight top divisions contribute
// 60 between them with uninformative per-league intervals). The top divisions
// stay in the observation tier — no stake, fully logged — until the Addendum 38
// group graduation trigger is met (pooled rule-16-clean population at the rule:
// n>=100, CI lower bound >0, no negative season block, no league individually
...
// sampling noise: domestic's Brier-minimum sits at 1.02 (settings.calibrationFactor,
// user-editable on the Settings tab, matches the domestic-only sweep almost exactly
// since domestic is ~92% of the pooled population anyway). Tournament's minimum is
// 1.06 — at 1.02 tournament calibration is measurably worse (0.24076) than at
// 1.06-1.08 (0.24022-0.24024), the opposite direction from domestic. Not exposed on
// the Settings tab (unlike the domestic value) — tournament leagues are always
// zero-stake today, so this only affects the accuracy of what gets logged for
...
const LEAGUES = {
  ...
  // Added 2026-09-04 (Phase 2 of the lower-division expansion, Addendum 42/43),
  // paper-only, observation tier (not in PAPER_STAKE_ELIGIBLE_LEAGUE_IDS), rule-10
  // protected from day one with the rule-15 conversion stated up front in
  // TRAINING_HOLDOUT_LEAGUE_IDS. Coverage confirmed in Addendum 42 Parts A-D:
  // API-Sports ids 136/141/79, Odds API keys active with Pinnacle on every
  // imminent-round fixture live and at five historical snapshots back to 2022,
  // teamsMatch() clean at fixture level (10/10, 12/12, 9/9).
  '136': { name: 'Serie B',               season: 2026, sport: 'soccer_italy_serie_b' },
  '141': { name: 'Segunda División',      season: 2026, sport: 'soccer_spain_segunda_division' },
  '79':  { name: '2. Bundesliga',         season: 2026, sport: 'soccer_germany_bundesliga2' },
};
...
async function scoreOneFixture(fix, formFixtures, standings, statsCache, oddsMap, settings, totalsMap = {}, opts = {}) {
  ...
  // Raw probs are preserved in modelProb for display. Domestic and tournament use
  // separate, separately-validated factors — see getCalFactorForLeague.
  ...
  const pocketTier = pocket ? pocket.tier : null;
  // A standalone-model pocket bets the standalone's own pick: substitute candidate
  // built from that pick's market row, with the standalone's probability, edge and a
  // paper-Kelly stake on its own calibrated probability (factor 1.0).
  let pocketCandidate = null;
  if (pocket && pocket.model === 'standalone' && standaloneShadow?.betLabel) {
    const row = results.find(r => r.bet === standaloneShadow.betLabel);
    if (row && row.hasRealOdds !== false && row.bookOdds > 1) {
      const k = kelly(standaloneShadow.calProb, row.bookOdds, settings.paperKellyFraction, getAvailableBankroll('paper'));
      pocketCandidate = { ...row, modelProb: standaloneShadow.prob, calibratedProb: standaloneShadow.calProb, edge: standaloneShadow.edge, ev: standaloneShadow.calProb * (row.bookOdds - 1) - (1 - standaloneShadow.calProb), kelly: k, pocketModelVersion: standaloneShadow.version, pocketModel: 'standalone' };
    }
  }
        ...
        try {
          ...
          if (shouldSurface) {
            ...
            watching.push({
              ...
              pocketTier:      scored.pocketTier ?? null,
...
async function runPreMatchScan(watchingEntry, overrides = {}) {
  ...
  try {
    ...
    const scored = await scoreOneFixture(fix, enrichedFormFixtures, standings, statsCache, oddsMap, settings, totalsMap);
    // A standalone-model pocket (League Two V2) bets the standalone's own pick.
    const best   = scored.pocketCandidate || (scored.isClassifiedLeague ? scored.tierCandidate
    ...
    // 2026-09-01: the temp lowconf-overlay-check diagnostic showed this gate, applied
    // to the domestic 18%/45% paper-money population specifically, drops the BETTER
    // half (n=910, ROI +35.8%, CI [19.8,51.8]) and keeps the worse half (n=1242, ROI
    // -1.0%, CI spans zero) — backwards from its intended purpose there. The
    // walk-forward robustness scan that validated 18%/45% in the first place never
    // applied this gate either (computeMatchedEdgeFixtures() doesn't compute
    // lowConfidence/dataConf/formCount at all), so skipping it for domestic leagues
    // restores parity with the population that was actually validated, instead of
    // silently betting a different, never-validated subset of it. Scope stays
    // exactly to what was tested: tournament and legacy/unclassified leagues keep
    // the gate untouched — no evidence gathered there either way.
    if (scored.lowConfidence && !scored.isDomesticTierLeague) {
    ...
    const realKelly = kelly(best.modelProb * getCalFactorForLeague(settings, leagueId),
    ...
    const bet   = {
      ...
      pocketId:     scored.pocketId ?? null,
      pocketTier:   scored.pocketTier ?? null,
      pocketModelVersion: best.pocketModelVersion ?? null,
      isTrainingHoldout: scored.isTrainingHoldout,
...
const HISTORICAL_BACKFILL_CONFIG = [
  ...
  // Serie B / Segunda División / 2. Bundesliga — added 2026-09-04 (Phase 2, paper-
  // only, rule-10 protected). Depth per API-Sports /leagues (Addendum 42 Part B):
  // Serie B and Segunda 2016-2026 (11 seasons), 2. Bundesliga 2011-2026 (16).
  { leagueId: '136', name: 'Serie B',          seasons: [2026, 2025, 2024, 2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016] },
  { leagueId: '141', name: 'Segunda División', seasons: [2026, 2025, 2024, 2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016] },
  { leagueId: '79',  name: '2. Bundesliga',    seasons: [2026, 2025, 2024, 2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012, 2011] },
];
...
// 2026-09-06 (Addendum 45 G4 / K): Championship, League One, League Two added so the
// post-match lineups backfill builds WOWY deltas for the staked cohort — without
...
function bucketStats(members) {
  ...
  return { n, open: n - resolved.length, resolved: resolved.length, wins, losses: resolved.length - wins,
    ...
    paperPnl: +resolved.filter(m => !m.real).reduce((a, m) => a + (m.bet.pnl || 0), 0).toFixed(2), paperStaked: +resolved.filter(m => !m.real).reduce((a, m) => a + (m.bet.actualStake ?? m.bet.suggestedStake ?? 0), 0).toFixed(2),
...
const CLOSING_ODDS_SPORT_MAP = {
  ...
  '40':  'soccer_efl_champ',
  // Added 2026-09-04 (Phase 2 lower-division expansion) — keys confirmed active
  // and historically populated back to Oct 2022 in Addendum 42 Part C.
...
const LEAGUE_NAMES_MAP = {
  ...
  '136': 'Serie B', '141': 'Segunda División', '79': '2. Bundesliga',
};
...
// Poisson pmf/cdf for total goals T ~ Poisson(lambda)
function _poisPmf(l, k) { let p = Math.exp(-l); for (let i = 1; i <= k; i++) p *= l / i; return p; }
function _poisCdf(l, k) { let s = 0; for (let i = 0; i <= k; i++) s += _poisPmf(l, i); return s; }
// Over-bet settlement on an Asian/half/integer line: returns { win, push, loss } probabilities under the model,
// and a settle(T) function giving the flat-stake P&L multiplier at odds o (quarter lines split the stake).
function _lineModel(lambda, line) {
  const half = (L) => ({ pOver: 1 - _poisCdf(lambda, Math.floor(L)), pPush: 0 });
  const integer = (L) => ({ pOver: 1 - _poisCdf(lambda, L), pPush: _poisPmf(lambda, L) });
  const parts = Number.isInteger(line * 2) ? [ (line % 1 === 0 ? integer : half)(line) ] : [ (Math.floor(line) % 1 === 0 && (line - Math.floor(line)) === 0.25) ? integer(Math.floor(line)) : half(Math.floor(line) + 0.5), (line - Math.floor(line)) === 0.25 ? half(Math.floor(line) + 0.5) : integer(Math.ceil(line)) ];
  const pOver = parts.reduce((a, p) => a + p.pOver, 0) / parts.length, pPush = parts.reduce((a, p) => a + p.pPush, 0) / parts.length;
  return { pOver, pPush, pOverCond: pOver / Math.max(1e-9, 1 - pPush) };
}
function _settleOver(T, line, odds) { // flat 1-unit P&L for an OVER bet
  const one = (L) => (L % 1 === 0 ? (T > L ? odds - 1 : T === L ? 0 : -1) : (T > L ? odds - 1 : -1));
  if (Number.isInteger(line * 2)) return one(line);
  const lo = Math.floor(line * 4) / 4; const a = (line - Math.floor(line)) === 0.25 ? [Math.floor(line), Math.floor(line) + 0.5] : [Math.floor(line) + 0.5, Math.ceil(line)];
  return (one(a[0]) + one(a[1])) / 2;
}
function _settleUnder(T, line, odds) { const one = (L) => (L % 1 === 0 ? (T < L ? odds - 1 : T === L ? 0 : -1) : (T < L ? odds - 1 : -1)); if (Number.isInteger(line * 2)) return one(line); const a = (line - Math.floor(line)) === 0.25 ? [Math.floor(line), Math.floor(line) + 0.5] : [Math.floor(line) + 0.5, Math.ceil(line)]; return (one(a[0]) + one(a[1])) / 2; }
...
app.get('/api/admin/research/totals-search', (req, res) => {
  try {
    ...
    for (const e of Object.values(store.entries)) {
      if (e.leagueId !== leagueId || !e.main || !(e.main.over > 1 && e.main.under > 1)) continue;
      if (req.query.line === '2.5' && e.main.point !== 2.5) continue; // optional: restrict to the 2.5 line
      ...
      const line = e.main.point; const lm = _lineModel(lamH + lamA, line); const pOverRaw = lm.pOverCond; // conditional on no push, the market's currency
      const io = 1 / e.main.over, iu = 1 / e.main.under; const mOver = io / (io + iu);
      const T = hg + ag; const halfLine = Math.abs(line * 2 - Math.round(line * 2)) < 1e-9 && line % 1 !== 0;
      rows.push({ fid: e.fixtureId, date: e.date, day, T, line, halfLine, over: halfLine ? (T > line ? 1 : 0) : (T > line ? 1 : T < line ? 0 : null), pOverRaw, mOver, oddsOver: e.main.over, oddsUnder: e.main.under, lam: lamH + lamA, pnlOver: _settleOver(T, line, e.main.over), pnlUnder: _settleUnder(T, line, e.main.under) });
    }
    ...
    const decided = (l) => l.filter(r => r.over === 0 || r.over === 1);
    const platt = _fitPlatt2(decided(train).map(r => r.pOverRaw), decided(train).map(r => r.over));
    ...
    // Bets on ALL lines settle exactly (pushes, quarter splits); the beyond-market residual is defined on half-line bets only (a decided outcome vs a no-vig probability).
    const evalCell = (list, side, eMin, pMin) => { const bets = list.filter(r => side === 'over' ? (r.edgeOver >= eMin - 1e-9 && r.pOver >= pMin - 1e-9) : (r.edgeUnder >= eMin - 1e-9 && (1 - r.pOver) >= pMin - 1e-9)).map(r => { const mk = side === 'over' ? r.mOver : 1 - r.mOver; const pnl = side === 'over' ? r.pnlOver : r.pnlUnder; const won = pnl > 0; const bm = r.halfLine ? ((won ? 1 : 0) - mk) : null; return { date: r.date, won, bm, pnl, mk, halfLine: r.halfLine, r }; }); const n = bets.length; if (!n) return { n: 0 }; const hb = bets.filter(b => b.bm != null); const bm = hb.length ? hb.reduce((a, b) => a + b.bm, 0) / hb.length : null, sd = hb.length > 1 ? Math.sqrt(hb.reduce((a, b) => a + (b.bm - bm) ** 2, 0) / (hb.length - 1)) : 0, pnl = bets.reduce((a, b) => a + b.pnl, 0); return { n, halfLineN: hb.length, wins: bets.filter(b => b.won).length, beyondMarketPp: bm != null ? +(bm * 100).toFixed(2) : null, sePp: (sd && hb.length) ? +((sd / Math.sqrt(hb.length)) * 100).toFixed(2) : null, z: (sd && hb.length) ? +(bm / (sd / Math.sqrt(hb.length))).toFixed(2) : null, roiClosePct: +((pnl / n) * 100).toFixed(1), unitsPerSeason: +(pnl / seasonsOf(list)).toFixed(1), betsPerSeason: +(n / seasonsOf(list)).toFixed(1), _bets: bets }; };
    const grid = [];
    for (const side of ['over', 'under']) for (let e = 0; e <= 0.15 + 1e-9; e += 0.01) for (let p = 0.35; p <= 0.70 + 1e-9; p += 0.05) { const t = evalCell(train, side, e, p); if (t.n >= selectN) grid.push({ side, edgeMin: +e.toFixed(2), probMin: +p.toFixed(2), train: { n: t.n, halfLineN: t.halfLineN, beyondMarketPp: t.beyondMarketPp, z: t.z, roiClosePct: t.roiClosePct, unitsPerSeason: t.unitsPerSeason, betsPerSeason: t.betsPerSeason } }); }
    const eligible = grid.filter(g => g.train.z != null && g.train.z >= minZ).sort((a, b) => b.train.unitsPerSeason - a.train.unitsPerSeason);
    ...
    const sub = (bets, f) => { const l = bets.filter(f); const n = l.length; if (!n) return { n: 0 }; const hb = l.filter(b => b.bm != null); const bm = hb.length ? hb.reduce((a, b) => a + b.bm, 0) / hb.length : null; return { n, beyondMarketPp: bm != null ? +(bm * 100).toFixed(1) : null, roiClosePct: +((l.reduce((a, b) => a + b.pnl, 0) / n) * 100).toFixed(1) }; };
    ...
    res.json({ leagueId, split, model: { type: 'rolling Poisson, independent goals, no Dixon–Coles; team window ' + N + ' matches, half-life ' + HALF + ', league window ' + LEAGUE_WIN, platt: { A: platt.A, B: platt.B } }, coverage: { entries: Object.values(store.entries).filter(e => e.leagueId === leagueId).length, withMainLine: Object.values(store.entries).filter(e => e.leagueId === leagueId && e.main).length, with25: Object.values(store.entries).filter(e => e.leagueId === leagueId && e.main?.point === 2.5).length, scoredRows: rows.length, halfLineRows: rows.filter(r => r.halfLine).length, lines: Object.entries(rows.reduce((a, r) => { a[r.line] = (a[r.line] || 0) + 1; return a; }, {})).sort((x, y) => y[1] - x[1]).slice(0, 8), trainN: train.length, testN: test.length, from: rows[0]?.date?.slice(0, 10), to: rows[rows.length - 1]?.date?.slice(0, 10) },
...
// ── Alternate totals backfill (research): Pinnacle's 2.5 line on EVERY fixture, per event, from 2023-05 ──
// Per (sport, kickoff minute): one historical /events call (1 credit) for event ids, then one
// historical event-odds call per matched fixture (10 credits, markets=alternate_totals, eu).
// Stores every Pinnacle alternate line plus the 2.5 line into research-alt-totals.json.
let _raStatus = { running: false };
async function runResearchAltTotalsBackfill({ budgetCredits = 60000, leagueIds = [41, 42], since = '2023-05-01', until = null } = {}) {
  const status = { running: true, startedAt: new Date().toISOString(), finishedAt: null, groupsTotal: 0, groupsDone: 0, groupsSkipped: 0, fixturesMatched: 0, fixturesMissed: 0, withPinnacleAlt: 0, with25: 0, creditsUsed: 0, creditsRemaining: null, error: null, budgetCredits, leagueIds, since };
  _raStatus = status; writeJSON('research-alt-status.json', status);
  const u = _researchUsage();
  try {
    const hist = readHistoricalCached() || {}; const store = readJSON('research-alt-totals.json') || { entries: {}, groupsDone: {} };
    const groups = new Map();
    for (const fix of (hist.fixtures || [])) {
      const lid = parseInt(fix.league?.id, 10); if (!leagueIds.includes(lid)) continue;
      const sport = RESEARCH_SPORTS[lid]; const date = fix.fixture?.date, fid = fix.fixture?.id; if (!sport || !date || !fid) continue;
    ...
    for (const [key, fixtures] of [...groups.entries()].sort()) {
      if (status.creditsUsed >= budgetCredits) { status.error = 'budget cap reached'; break; }
      if (store.groupsDone[key] && store.groupsDone[key] !== 'error') { status.groupsSkipped++; continue; }
      const [sport, minuteKey] = key.split('|'); const kickoffIso = `${minuteKey}:00Z`;
      let evr;
      try { evr = await oddsApi.get(`/historical/sports/${sport}/events`, { params: { apiKey: ODDS_API_KEY, date: kickoffIso } }); }
      catch (e) { status.error = `${key} events: ${e.response?.status || ''} ${(e.response?.data?.message || e.message || '').slice(0, 120)}`; if (e.response?.status === 429) { await new Promise(r => setTimeout(r, 5000)); continue; } if ([401, 402].includes(e.response?.status)) break; store.groupsDone[key] = 'error'; continue; }
      _logOddsCall(u, `research alt events ${sport} ${kickoffIso}`, evr.headers); status.creditsUsed += parseInt(evr.headers['x-requests-last'] || '0', 10) || 0;
      const events = evr.data?.data || [];
      for (const fix of fixtures) {
        const home = fix.teams?.home?.name, away = fix.teams?.away?.name, fid = fix.fixture?.id;
        const cands = events.filter(e => teamsMatch(e.home_team, home) && teamsMatch(e.away_team, away));
        const ev = _pickByKickoff(cands, fix.fixture?.date, e => e.commence_time, () => 1) || cands[0] || null;
        if (!ev) { status.fixturesMissed++; continue; }
        let r;
        try { r = await oddsApi.get(`/historical/sports/${sport}/events/${ev.id}/odds`, { params: { apiKey: ODDS_API_KEY, regions: 'eu', markets: 'alternate_totals', oddsFormat: 'decimal', date: kickoffIso } }); }
        catch (e) { status.error = `${key} ${fid}: ${e.response?.status || ''} ${(e.response?.data?.message || e.message || '').slice(0, 120)}`; if (e.response?.status === 429) { await new Promise(r => setTimeout(r, 5000)); } continue; }
        _logOddsCall(u, `research alt odds ${sport} ${kickoffIso}`, r.headers); status.creditsUsed += parseInt(r.headers['x-requests-last'] || '0', 10) || 0; status.creditsRemaining = parseInt(r.headers['x-requests-remaining'] || '0', 10) || status.creditsRemaining;
        if (status.creditsRemaining && status.creditsRemaining <= ODDS_CREDITS_RESERVE) { status.error = 'reserve line reached'; break; }
        status.fixturesMatched++;
        const bks = r.data?.data?.bookmakers || []; const pin = bks.find(b => b.key === 'pinnacle'); const m = pin?.markets?.find(x => x.key === 'alternate_totals');
        const byPoint = {}; for (const o of (m?.outcomes || [])) { const k = String(o.point); byPoint[k] = byPoint[k] || {}; byPoint[k][o.name.toLowerCase()] = o.price; }
        const lines = Object.entries(byPoint).filter(([, v]) => v.over > 1 && v.under > 1).map(([point, v]) => ({ point: +point, over: v.over, under: v.under, margin: +((1 / v.over + 1 / v.under - 1).toFixed(4)) })).sort((a, b) => a.point - b.point);
        const l25 = lines.find(l => l.point === 2.5) || null; const otherBooks25 = bks.filter(b => b.key !== 'pinnacle' && b.markets?.some(x => x.key === 'alternate_totals' && x.outcomes?.some(o => o.point === 2.5))).length;
        if (lines.length) status.withPinnacleAlt++; if (l25) status.with25++;
        store.entries[fid] = { fixtureId: fid, leagueId: parseInt(fix.league?.id, 10), date: fix.fixture?.date, home, away, goals: { home: fix.goals?.home ?? fix.score?.fulltime?.home, away: fix.goals?.away ?? fix.score?.fulltime?.away }, snapshotTs: r.data?.timestamp || kickoffIso, eventId: ev.id, pinnacleAltLines: lines, line25: l25, otherBooks25 };
        await new Promise(r => setTimeout(r, 120));
      }
      if (status.error === 'reserve line reached') break;
      store.groupsDone[key] = new Date().toISOString(); status.groupsDone++;
      if (status.groupsDone % 10 === 0) { writeJSON('research-alt-totals.json', store); writeJSON('research-alt-status.json', status); }
    }
    writeJSON('research-alt-totals.json', store);
  } catch (e) { status.error = e.message; }
  status.running = false; status.finishedAt = new Date().toISOString(); writeJSON('research-alt-status.json', status);
  console.log(`[ResearchAlt] done — groups ${status.groupsDone}/${status.groupsTotal}, matched ${status.fixturesMatched}, pinnacle alt ${status.withPinnacleAlt}, 2.5 line ${status.with25}, credits ${status.creditsUsed}${status.error ? ', error: ' + status.error : ''}`);
  return status;
}
app.post('/api/admin/research/alt-totals-backfill', (req, res) => {
  if (_raStatus.running) return res.json({ started: false, status: _raStatus });
  const opts = { budgetCredits: parseInt(req.query.budget || '60000', 10), leagueIds: (req.query.leagues || '41,42').split(',').map(Number), since: req.query.since || '2023-05-01', until: req.query.until || null };
  runResearchAltTotalsBackfill(opts).catch(e => console.error(`[ResearchAlt] ${e.message}`));
  res.json({ started: true, opts });
});
...
app.post('/api/admin/research/totals-pocket', (req, res) => {
  ...
  const modelOnly = req.query.modelOnly === '1'; const tag = req.query.tag ? `-${String(req.query.tag).replace(/[^a-z0-9-]/gi, '')}` : '';
  require('./research/totalsPocket').run({ dataDir: DATA_DIR, leagueId: lid, windows, inRealPocket, seed: parseInt(req.query.seed || '20260922', 10), modelOnly })
    .then(out => { writeJSON(`research-totals-pocket-${lid}${tag}.json`, out); _tpStatus[lid] = { running: false, finishedAt: new Date().toISOString(), seconds: out.seconds }; console.log(`[ResearchTotalsPocket] league ${lid} done in ${out.seconds}s`); })
...
app.get('/api/admin/diag-standalone-forward', async (req, res) => {
  try {
    const leagueId = parseInt(req.query.league, 10) || 42;
    // 2026-09-21: ?modelKey=archive:<league>:<tag> reads an archived candidate through
    // the same chain; ?from= overrides the window start (walk-forward holdout reads).
    const modelKey = req.query.modelKey || String(leagueId);
    const cutoff = req.query.from || DATE_SPLIT_HOLDOUT_CUTOFFS.get(leagueId) || '2026-08-11T09:00:00Z';
    const { classifyFixture, WEIGHTS_BY_CONTEXT, CONTEXT_CONFIG, LEAGUE_CONFIG: LC } = require('./scoring');
    const settings = getSettings();
    const hist = readHistoricalCached() || {};
    const closing = getClosingOdds();
    ...
    for (const r of recs) {
      ...
      const sp = standaloneChainProbs(modelKey, leagueId, r, context);
      if (!sp) continue;
      { const rawSp = model.predictLeague(modelKey, r.homeFactors, r.awayFactors, context); const dd = rawSp ? Math.max(...['home', 'draw', 'away'].map(k => Math.abs(sp.probs[k] - rawSp.probs[k]))) : null; if (dd != null && dd > chainVsRawMaxDiff) chainVsRawMaxDiff = dd; } // 2026-09-21: chain (standalone template) vs raw model — 0 while every stage is off
      const stripped = marginStrippedImplied(co);
      ...
      rows.push({ date: r.date, fixture: `${r.homeTeamName} v ${r.awayTeamName}`, fixtureId: r.fixtureId, outcome: r.actualOutcome, close: { home: co.homeOdds, draw: co.drawOdds, away: co.awayOdds }, pooled: mk(pooled, fP), standalone: mk(sp.probs, fS), llDiff: -Math.log(Math.max(1e-12, sp.probs[r.actualOutcome])) + Math.log(Math.max(1e-12, pooled[r.actualOutcome])), homeWon: r.actualOutcome === 'home' ? 1 : 0, homeS: sp.probs.home, homeP: pooled.home });
    }
    ...
    const ll = rows.map(r => r.llDiff); const lln = ll.length, llm = lln ? ll.reduce((a, b) => a + b, 0) / lln : null, llsd = lln > 1 ? Math.sqrt(ll.reduce((a, d) => a + (d - llm) ** 2, 0) / (lln - 1)) : null;
    const pairedLogLoss = lln ? { n: lln, meanDiff: +llm.toFixed(5), se: llsd ? +(llsd / Math.sqrt(lln)).toFixed(5) : null, z: llsd ? +(llm / (llsd / Math.sqrt(lln))).toFixed(2) : null, note: 'standalone − pooled log-loss per fixture; negative = standalone better', homeActual: +(rows.reduce((a, r) => a + r.homeWon, 0) / lln).toFixed(3), homeExpectedStandalone: +(rows.reduce((a, r) => a + r.homeS, 0) / lln).toFixed(3), homeExpectedPooled: +(rows.reduce((a, r) => a + r.homeP, 0) / lln).toFixed(3) } : null;
    ...
    res.json({ leagueId, modelKey, cutoff, matchedForward: rows.length, standaloneVersion: model.predictLeague(modelKey, {}, {}, 'club_domestic')?.version ?? null, chain: chainOptionsFor(String(leagueId)), chainVsRawMaxDiff, pairedLogLoss,
...
function getCalibrationCohorts() {
  ...
  const rows = Object.keys(LEAGUE_CONFIG).map(id => { const lid = parseInt(id, 10); return {
    ...
    pockets: POCKETS.filter(p => p.leagueId === lid).map(p => ({ id: p.id, tier: p.tier, label: p.label, edgeMin: p.edgeMin, probMin: p.probMin, months: p.months, priority: p.priority, from: p.from, model: p.model || 'pooled' })),
  }; });
  const cohortMap = {};
  for (const r of rows) {
