...
// TEMPORARY DIAGNOSTIC — soft-book coverage-gap + edge-direction check.
// For each named league: samples real fixtures Pinnacle already matched
// ("overlap") and real fixtures within the 2020+ window Pinnacle missed
// ("gap"), one historical odds call per sampled fixture (returns every
// bookmaker present in that single response — no per-book call needed).
// On overlap fixtures, computes model-implied edge against Pinnacle and
// against each candidate soft book for correlation/ranking checks.
// Read-only except for the Odds API calls it exists to make. To be removed
// after review.
app.get('/api/debug/softbook-coverage-check', async (_req, res) => {
  ...
  const CLOSING_ODDS_SPORT_MAP_LOCAL = CLOSING_ODDS_SPORT_MAP;

  const SOFT_BOOKS = ['williamhill', 'ladbrokes_uk', 'unibet_uk'];
  const TARGET_LEAGUES = [88, 848]; // Eredivisie, Conference League
  const ODDS_FLOOR = new Date('2020-06-06T10:05:00Z');
  const SAMPLE_PER_BUCKET = 12; // per league, per gap/overlap bucket

  function pickSpread(list, n) {
    // Even spread across the sorted-by-date list, not just the first n —
    // avoids over-sampling one season.
  ...
  for (const leagueId of TARGET_LEAGUES) {
    const sport = CLOSING_ODDS_SPORT_MAP_LOCAL[String(leagueId)];
    const leagueRecords = scoredRecords
      .filter(r => parseInt(r.leagueId, 10) === leagueId && r.actualOutcome && r.homeFactors && r.awayFactors && r.date && new Date(r.date) >= ODDS_FLOOR)
      .sort((a, b) => new Date(a.date) - new Date(b.date));

    const overlapAll = leagueRecords.filter(r => closingOdds[r.fixtureId] || closingOdds[String(r.fixtureId)]);
    const gapAll      = leagueRecords.filter(r => !closingOdds[r.fixtureId] && !closingOdds[String(r.fixtureId)]);

    const overlapSample = pickSpread(overlapAll, SAMPLE_PER_BUCKET);
    const gapSample      = pickSpread(gapAll, SAMPLE_PER_BUCKET);

    const leagueResult = {
      leagueId, name: LEAGUE_CONFIG[leagueId]?.name, sport,
      totalInWindow: leagueRecords.length, pinnacleMatchedInWindow: overlapAll.length, pinnacleGapInWindow: gapAll.length,
      overlapChecked: [], gapChecked: [],
    };

    async function queryFixture(rec) {
      const kickoffIso = new Date(rec.date).toISOString().split('.')[0] + 'Z';
      try {
        const resp = await oddsApi.get(`/historical/sports/${sport}/odds`, {
        ...
        apiCalls++;
        const events = resp.data?.data || resp.data || [];
        // scoreFixtureFromPool() stores homeTeamName/awayTeamName directly on the record.
        const ev = events.length === 1 ? events[0] : events.find(e =>
          rec.homeTeamName && rec.awayTeamName && teamsMatch(e.home_team, rec.homeTeamName) && teamsMatch(e.away_team, rec.awayTeamName)
        );
        if (!ev) return { fixtureId: rec.fixtureId, date: rec.date, matched: false, eventCountAtSnapshot: events.length };
        const bookmakerKeys = (ev.bookmakers || []).map(b => b.key);
        const pricesFor = key => {
          const bm = ev.bookmakers?.find(b => b.key === key);
          const mkt = bm?.markets?.find(m => m.key === 'h2h');
          if (!mkt) return null;
          const get = name => mkt.outcomes?.find(o => teamsMatch(o.name, name))?.price ?? null;
          return { homeOdds: get(ev.home_team), drawOdds: get('Draw'), awayOdds: get(ev.away_team) };
        };
        return {
          fixtureId: rec.fixtureId, date: rec.date, matched: true,
          eventTeams: `${ev.home_team} vs ${ev.away_team}`,
          bookmakerKeys,
          pinnacle: pricesFor('pinnacle'),
          softBooks: Object.fromEntries(SOFT_BOOKS.filter(k => bookmakerKeys.includes(k)).map(k => [k, pricesFor(k)])),
        };
      } catch (e) {
        apiCalls++;
        return { fixtureId: rec.fixtureId, date: rec.date, error: e.response?.status || e.message };
      }
    }

    for (const rec of gapSample) {
      const r = await queryFixture(rec);
      leagueResult.gapChecked.push(r);
      await new Promise(res2 => setTimeout(res2, 250));
    }
    for (const rec of overlapSample) {
      const r = await queryFixture(rec);
      // Compute model probability + implied edge vs Pinnacle and vs each soft book present
      if (r.matched) {
        ...
        if (weights) {
          ...
          const oddsFor = (prices) => {
            if (!prices) return null;
            return topOutcome === 'home' ? prices.homeOdds : topOutcome === 'away' ? prices.awayOdds : prices.drawOdds;
          };
          const edgeFor = (prices) => {
            const odds = oddsFor(prices);
            if (!odds || odds <= 1) return null;
            const implied = 1 / odds;
            return +((modelProb - implied) / implied).toFixed(4);
          };
          r.modelProb = +modelProb.toFixed(4);
          r.topOutcome = topOutcome;
          r.edgeVsPinnacle = edgeFor(r.pinnacle);
          r.edgeVsSoftBook = Object.fromEntries(Object.entries(r.softBooks).map(([k, p]) => [k, edgeFor(p)]));
        }
      }
      leagueResult.overlapChecked.push(r);
      await new Promise(res2 => setTimeout(res2, 250));
    }
  ...
  res.json({ softBooksChecked: SOFT_BOOKS, results, apiCallsMade: apiCalls });
});
