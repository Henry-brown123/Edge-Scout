...
// TEMP DIAGNOSTIC — remove after use. User asked whether ANY genuine
// prospective check on edge>=18%/modelProb>=45% is possible right now,
// without waiting. Two things ARE checkable without any wait, because
// neither was touched by the backtest mining that found the rule: (1) any
// fixtures dated after the concurrent-coverage pool's own latest date
// (2026-08-17) for the 8 leagues whose test sets keep growing weekly
// (VALIDATED_SPLITS, rule 9) — genuinely never-seen data, however little of
// it exists yet; (2) the live paper/real bet log itself -- bets already
// locked via the completely separate successScore gate that happen to also
// satisfy this rule, using live-at-lock odds rather than reconstructed
// closing prices. Both are likely far too thin to prove anything alone --
// reported honestly as such, not oversold.
app.get('/api/admin/diag-prospective-check', async (_req, res) => {
  try {
    const matched = await computeMatchedEdgeFixtures();
    const DOMESTIC = new Set([39, 140, 135, 78, 61, 179, 88, 94, 41, 42, 40]);
    const RULE9_ONLY = new Set([39, 140, 135, 78, 61, 179, 88, 94]); // growing test sets, no date-split cutoff

    // (1) Fresh data: fixtures after the prior pool's known latest date, for
    // the rule-9 (growing test set) leagues only.
    const PRIOR_POOL_LATEST = new Date('2026-08-17T23:59:59Z');
    const freshFixtures = matched.filter(f => {
      const lid = parseInt(f.leagueId, 10);
      if (!RULE9_ONLY.has(lid)) return false;
      const split = VALIDATED_SPLITS[lid];
      if (!split || new Date(f.date) < new Date(split.testFrom)) return false; // still respect the original test-only cutoff
      return new Date(f.date) > PRIOR_POOL_LATEST;
    });
    const freshCell = freshFixtures.filter(f => f.edge >= 0.18 && f.modelProb >= 0.45);
    ...
    const freshResult = pooledCi(freshCell.map(f => f.won ? (f.pinnacleOdds - 1) : -1));

    // (2) Live bet log cross-check: bets already locked via the normal
    // successScore-gated pipeline (independent of this analysis) that also
    // clear edge>=18%/modelProb>=45%, domestic leagues, resolved only.
    const bets = getBets().filter(b => {
      const lid = parseInt(b.leagueId, 10);
      if (!DOMESTIC.has(lid)) return false;
      if (!b.result) return false; // unresolved — can't score yet
      return (b.edge ?? -1) >= 0.18 && (b.modelProb ?? 0) >= 0.45;
    });
    const liveOdds = bets.map(b => b.actualOdds ?? b.bookOdds);
    const liveReturns = bets.map((b, i) => b.result === 'win' ? (liveOdds[i] - 1) : -1);
    const liveResult = pooledCi(liveReturns);

    res.json({
      note: 'TEMP diagnostic — two genuinely non-circular checks on edge>=18%/modelProb>=45%, neither requiring a wait. freshDataCheck: fixtures dated after the prior mining pool\'s latest date (2026-08-17), for the 8 growing-test-set leagues only -- real, never-touched data, however little exists yet. liveBetLogCheck: already-placed paper/real bets (domestic leagues, resolved only) that happen to satisfy the rule, locked via the completely separate successScore gate using live-at-lock odds. Both are honestly reported even if too thin to mean much -- this is NOT a substitute for real prospective volume accumulating over the coming weeks/months. Delete this endpoint once read.',
      freshDataCheck: { latestPriorPoolDate: '2026-08-17', freshFixturesTotal: freshFixtures.length, cellN: freshCell.length, ...freshResult, fixtures: freshCell.map(f => ({ leagueId: f.leagueId, date: f.date, won: f.won, edge: +(f.edge*100).toFixed(1), modelProb: +(f.modelProb*100).toFixed(1) })) },
      liveBetLogCheck: { qualifyingBets: bets.length, ...liveResult, bets: bets.map(b => ({ fixture: b.fixture, leagueName: b.leagueName, kickoff: b.kickoff, result: b.result, edge: b.edge != null ? +(b.edge*100).toFixed(1) : null, modelProb: b.modelProb != null ? +(b.modelProb*100).toFixed(1) : null, actualOdds: b.actualOdds ?? b.bookOdds })) },
    });
  } catch (e) {
