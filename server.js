...
async function scoreOneFixture(fix, formFixtures, standings, statsCache, oddsMap, settings, totalsMap = {}) {
  ...
  return {
    ...
    isClassifiedLeague, isDomesticTierLeague, tierCandidate, meetsPaperMoneyRule, isFakeMoney,
...
// ─── TEMP DIAGNOSTIC (2026-09-04, Addendum 39) — remove after use ──────────────
// From-scratch threshold review for Championship / League One / League Two only.
// Stage 1: full edge-floor × probability-BAND grid on TRAIN (reference view, no
// selection). Stage 2: probability floors 40/45/50% × edge floors on TRAIN, with two
// pre-registered picks per floor — max absolute return, and max CI lower bound
// (risk-adjusted) — both requiring train n>=100 and >=2 positive train blocks.
// Stage 3: TEST figures are computed ONLY for the distinct picked cells (plus the
// incumbent 18/45, whose test figure is already published in Addendum 38); no test
// grid is ever built. Same split as Addendum 38: train < 2024-09-16 <= test < cutoff.
app.get('/api/admin/diag-rule12-full-threshold-review', async (_req, res) => {
  try {
    const { LEAGUE_CONFIG } = require('./scoring');
    const RULE12 = { 41: '2026-08-11T09:00:00Z', 42: '2026-08-11T09:00:00Z', 40: '2026-08-19T22:00:00Z' };
    const TRAIN_END = '2024-09-16T00:00:00Z';
    const matched = await computeMatchedEdgeFixtures();
    const pop = matched.filter(f => RULE12[parseInt(f.leagueId, 10)] && new Date(f.date) < new Date(RULE12[parseInt(f.leagueId, 10)]))
      .sort((a, b) => new Date(a.date) - new Date(b.date));
    const train = pop.filter(f => new Date(f.date) < new Date(TRAIN_END));
    const test  = pop.filter(f => new Date(f.date) >= new Date(TRAIN_END));

    const stats = arr => {
      const n = arr.length;
      if (!n) return { n: 0, roi: null, ci95: null, ciLow: null, absReturn: 0, thin: true };
      ...
      return { n, wins: arr.filter(f => f.won).length, roi: +(mean * 100).toFixed(2), ci95: [+((mean - half) * 100).toFixed(2), +((mean + half) * 100).toFixed(2)], ciLow: +((mean - half) * 100).toFixed(2),
        absReturn: +(rets.reduce((s, v) => s + v, 0)).toFixed(1), avgOdds: +(arr.reduce((s, f) => s + f.pinnacleOdds, 0) / n).toFixed(2), thin: n < 30 };
    };
    ...
    const trainBlocks = blocksOf(train), testBlocks = blocksOf(test);
    const rangeOf = b => b.length ? `${b[0].date.slice(0, 10)} to ${b[b.length - 1].date.slice(0, 10)}` : 'empty';

    // Stage 1 — full band × edge-floor grid on train
    const BANDS = ['<35%', '35-40%', '40-45%', '45-50%', '50-55%', '55-60%', '60-65%', '65-70%', '70-75%', '75-80%', '80%+'];
    const EDGE1 = [-1, 0, 0.05, 0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.24];
    const stage1 = {};
    for (const band of ['all', ...BANDS]) {
      const inBand = band === 'all' ? train : train.filter(f => tierOfProbShared(f.modelProb) === band);
      stage1[band] = Object.fromEntries(EDGE1.map(e => [e < 0 ? 'any' : `>=${Math.round(e * 100)}%`, stats(inBand.filter(f => e < 0 || f.edge >= e))]));
    }
    const stage1ByLeague = Object.fromEntries(['Championship', 'League One', 'League Two'].map(L => [L, Object.fromEntries(['all', ...BANDS].map(band => {
      const inBand = train.filter(f => name(f) === L && (band === 'all' || tierOfProbShared(f.modelProb) === band));
      return [band, { any: stats(inBand), ge18: stats(inBand.filter(f => f.edge >= 0.18)) }];
    }))]));

    // Stage 2 — floors × edge floors on train, two pre-registered picks per floor
    const FLOORS = [0.40, 0.45, 0.50];
    const EDGE2 = [0.08, 0.10, 0.12, 0.14, 0.15, 0.16, 0.18, 0.20, 0.22, 0.24];
    const passes = (f, e, p) => f.edge >= e && f.modelProb >= p;
    const stage2 = {};
    const candidates = [];
    for (const p of FLOORS) {
      const cells = EDGE2.map(e => {
        const cell = train.filter(f => passes(f, e, p));
        const perBlock = trainBlocks.map(b => stats(b.filter(f => passes(f, e, p))));
        const st = stats(cell);
        return { edge: e, prob: p, ...st, tStat: st.n > 1 && st.ci95 ? +((st.roi) / ((st.ci95[1] - st.ci95[0]) / 3.92 || 1)).toFixed(2) : null,
          perBlock: perBlock.map(b => ({ n: b.n, roi: b.roi })), positiveBlocks: perBlock.filter(b => b.n >= 20 && b.roi > 0).length };
      });
      const eligible = cells.filter(c => c.n >= 100 && c.positiveBlocks >= 2);
      const returnPick = eligible.slice().sort((a, b) => (b.absReturn - a.absReturn) || (b.edge - a.edge))[0] || null;
      const safetyPick = eligible.slice().sort((a, b) => (b.ciLow - a.ciLow) || (b.edge - a.edge))[0] || null;
      stage2[`>=${Math.round(p * 100)}%`] = { cells, returnPick: returnPick && { edge: returnPick.edge, prob: p, n: returnPick.n, roi: returnPick.roi, ci95: returnPick.ci95, absReturn: returnPick.absReturn, blocks: returnPick.perBlock.map(b => b.roi) },
        safetyPick: safetyPick && { edge: safetyPick.edge, prob: p, n: safetyPick.n, roi: safetyPick.roi, ci95: safetyPick.ci95, absReturn: safetyPick.absReturn, blocks: safetyPick.perBlock.map(b => b.roi) } };
      for (const c of [returnPick, safetyPick]) if (c && !candidates.some(x => x.edge === c.edge && x.prob === p)) candidates.push({ edge: c.edge, prob: p });
    }

    // Stage 3 — single test look, only for the distinct candidates (+ incumbent, already published)
    const testLook = (e, p) => { const cell = test.filter(f => passes(f, e, p)); return { edge: e, prob: p, pooled: stats(cell), byLeague: group(cell, name),
      perBlock: testBlocks.map(b => { const st = stats(b.filter(f => passes(f, e, p))); return { range: rangeOf(b), n: st.n, roi: st.roi, ci95: st.ci95 }; }) }; };
    const isIncumbent = c => c.edge === PAPER_MONEY_EDGE_MIN && c.prob === PAPER_MONEY_PROB_MIN;
    const testLooks = candidates.map(c => ({ ...testLook(c.edge, c.prob), pickedFor: FLOORS.map(p => `>=${Math.round(p * 100)}%`).filter(k => ['returnPick', 'safetyPick'].some(t => stage2[k][t] && stage2[k][t].edge === c.edge && stage2[k][t].prob === c.prob)).join(', ') || null, isIncumbent: isIncumbent(c) }));
    const incumbentAlreadyLooked = candidates.some(isIncumbent);
    const incumbent = incumbentAlreadyLooked ? null : { ...testLook(PAPER_MONEY_EDGE_MIN, PAPER_MONEY_PROB_MIN), note: 'reference — this figure was already published in Addendum 38, not a new look' };

    res.json({
      note: 'TEMP — Addendum 39. Rule-12 leagues only. Stage 1/2 on train; Stage 3 test figures only for the pre-registered candidates. Remove after use.',
      scoringModel: getModelTreeBoundary(), calFactor: getCalFactorForLeague(getSettings(), 40),
      population: { total: pop.length, train: { n: train.length, from: train[0]?.date.slice(0, 10), to: train[train.length - 1]?.date.slice(0, 10), blocks: trainBlocks.map(rangeOf) }, test: { n: test.length, from: test[0]?.date.slice(0, 10), to: test[test.length - 1]?.date.slice(0, 10), blocks: testBlocks.map(rangeOf) } },
      selectionRule: 'per floor: train n>=100 and >=2 positive equal-count train blocks (n>=20); returnPick = max absolute return; safetyPick = max 95% CI lower bound; ties → higher edge floor. Test computed only for distinct picks.',
      stage1: { grid: stage1, byLeague: stage1ByLeague },
      stage2,
      stage3: { candidates, testLooks, incumbentReference: incumbent },
    });
  } catch (e) {
    res.status(500).json({ error: e.message, stack: e.stack });
  }
