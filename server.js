...
// TEMPORARY diagnostic endpoint — retrospective scoring with the enriched (backfill-blended)
// form pool the fixed runPreMatchScan would now use. Read-only: no writes to bets.json,
// fixture-stats.json, lineups.json, or calibration.json.
app.get('/api/debug/retro-score', async (req, res) => {
  try {
    ...
    const leagueId = String(fix.league?.id);
    const meta = LEAGUES[leagueId] || { season: fix.league?.season || 2024, name: fix.league?.name };
    const settings = getSettings();

    // Live form fetch — same query shape as runPreMatchScan
    const formSeasons = [meta.season, meta.season - 1];
    const formResults = await Promise.all(
      formSeasons.map(s => apiSports.get('/fixtures', { params: { league: leagueId, season: s, last: 60 } }).catch(() => ({ data: { response: [] } })))
    );
    const rawFormFixtures = formResults.flatMap(r => r.data?.response || [])
      .filter(f => f.fixture?.status?.short === 'FT')
      .sort((a, b) => new Date(b.fixture?.date) - new Date(a.fixture?.date));

    // Enriched pool — the fix just ported to runPreMatchScan
    const backfillData = readJSON('backfill-historical.json') || { fixtures: [] };
    const backfillFixtures = (backfillData.fixtures || []).filter(f => f.fixture?.status?.short === 'FT');
    const leagueBackfill = backfillFixtures.filter(f => String(f.league?.id) === String(leagueId));
    const enrichedFormFixtures = [...rawFormFixtures, ...leagueBackfill]
    ...
    // Stats cache from disk only — no live fetch, keeps this endpoint read-only
    const fixtureStatsDb = getFixtureStats();
    const statsCache = {};
    for (const f of enrichedFormFixtures) {
      const s = fixtureStatsDb[String(f.fixture?.id)];
      if (s) statsCache[f.fixture.id] = s;
    }

    // Synthetic oddsMap built from the stored Pinnacle closing line — the live odds API
    // no longer has this fixture since it already kicked off.
    const closingOdds = readJSON('closing-odds.json') || {};
    const co = closingOdds[fixtureId] || closingOdds[String(fixtureId)];
    const homeName = fix.teams?.home?.name;
    const awayName = fix.teams?.away?.name;
    let oddsMap = {};
    if (co && co.homeOdds && co.drawOdds && co.awayOdds) {
      const prices = { [homeName]: co.homeOdds, Draw: co.drawOdds, [awayName]: co.awayOdds };
      const total = 1 / co.homeOdds + 1 / co.drawOdds + 1 / co.awayOdds;
      const stripped = { [homeName]: (1 / co.homeOdds) / total, Draw: (1 / co.drawOdds) / total, [awayName]: (1 / co.awayOdds) / total };
      oddsMap[`${homeName}|${awayName}`] = { ...prices, _pinnacleStripped: stripped, _pinnacleRaw: prices };
    }

    const scored = await scoreOneFixture(fix, enrichedFormFixtures, standings, statsCache, oddsMap, settings, {});

    const { CONTEXT_CONFIG } = require('./scoring');
    const cfg = CONTEXT_CONFIG[scored.context] || CONTEXT_CONFIG.club_domestic;
    const gapThreshold = Math.max(0, cfg.gapThresholdBase - (1 - scored.dataConf) * 0.15);
    const effectiveGapThreshold = (scored.context === 'international' && scored.competitionPhase === 'knockout')
      ? Math.min(gapThreshold, 0.10) : gapThreshold;

    const matchOutcomeResults = scored.results.filter(r => r.market === 'match_outcome');
    const best = matchOutcomeResults.reduce((a, b) => a.successScore > b.successScore ? a : b);
    const threshold = settings.successThreshold || 40;
    const wouldHaveLocked = best.successScore >= threshold && !scored.lowConfidence;

    let actualResult = null;
    if (fix.goals?.home != null && fix.goals?.away != null) {
      actualResult = fix.goals.home > fix.goals.away ? 'Home Win' : fix.goals.home < fix.goals.away ? 'Away Win' : 'Draw';
    }

    let hypothetical = null;
    if (wouldHaveLocked && actualResult) {
      const stake = best.kelly?.stake || 0;
      const won   = best.bet === actualResult;
      hypothetical = { stake, bet: best.bet, odds: best.bookOdds, won, pnl: won ? stake * (best.bookOdds - 1) : -stake };
    }

    res.json({
      fixture: `${homeName} vs ${awayName}`,
      fixtureId, leagueId, leagueName: meta.name,
      formPoolSizes: {
        rawLiveFetchTotal: rawFormFixtures.length,
        enrichedTotal: enrichedFormFixtures.length,
        homeFormCountEnriched: scored.homeFormCount,
        awayFormCountEnriched: scored.awayFormCount,
        minFormCountEnriched: scored.minFormCount,
      },
      factorBreakdown: { home: scored.homeF, away: scored.awayF },
      candidates: matchOutcomeResults,
      bestPick: { bet: best.bet, successScore: best.successScore, modelProb: best.modelProb, bookOdds: best.bookOdds, edge: best.edge },
      confidenceCheck: {
        lowConfidence: scored.lowConfidence,
        lowConfidenceReason: scored.lowConfidenceReason,
        maxModelBookGap: scored.maxModelBookGap,
        gapThreshold,
        effectiveGapThreshold,
        tierThreshold: scored.tierThreshold,
        minFormCount: scored.minFormCount,
        dataConf: scored.dataConf,
      },
      pinnacleOddsUsed: co || null,
      actualResult,
      actualScore: fix.goals,
      lockThreshold: threshold,
      wouldHaveLocked,
      hypothetical,
      caveats: [
        'Weather is unavailable for a past kickoff (forecast API only covers the next 7 days) — neutral default applied.',
        'Injuries endpoint reflects current API-Sports data, not necessarily the exact T-60 snapshot.',
        'Stats cache loaded from disk only, not live-refetched, to keep this endpoint read-only.',
      ],
    });
  } catch (e) {
