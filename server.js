...
// TEMP diagnostic — re-cuts Addendum 14/15's holdout-window population against a
// lower (1%) edge threshold, plus a 1/2/3/5% sensitivity table. Reuses the EXACT
// same population/model/methodology (no new odds/API calls, no re-fit, same
// single test-set look, re-sliced). See docs/tier-calibration-analysis.md
// Addendum 14/15 extension for the write-up this feeds.
app.get('/api/debug/threshold-sensitivity', (_req, res) => {
  try {
    ...
    function statsFor(pop) {
      const n = pop.length;
      if (!n) return { n: 0, roi: null, ci95: null, sd: null };
      ...
      // n needed for the CI to exclude zero at the current point estimate, holding
      // mean and sd fixed: half-width = 1.96*sd/sqrt(n) < |mean|  =>  n > (1.96*sd/mean)^2
      const nNeededForSignificance = (mean !== 0 && n > 1) ? Math.ceil((1.96 * sd / Math.abs(mean)) ** 2) : null;
      return { n, roi: +(mean * 100).toFixed(1), ci95, sd: +sd.toFixed(3), thin: n < 30, nNeededForSignificance };
    }

    // ── 1% threshold: full cleared/blocked stats per tier, pooled + per league ──
    const pooledAt1Pct = CALIB_TIER_LABELS.map(label => {
      const t = TIERS.find(x => x.label === label);
      const inTier = holdoutMatched.filter(r => r.modelProb >= t.lo && r.modelProb < t.hi);
      const cleared = inTier.filter(r => r.edge >= 0.01);
      const blocked = inTier.filter(r => r.edge < 0.01);
      return { tier: label, totalN: inTier.length, cleared: statsFor(cleared), blocked: statsFor(blocked) };
    });

    const byLeagueAt1Pct = {};
    for (const lid of validatedIds) {
      byLeagueAt1Pct[lid] = CALIB_TIER_LABELS.map(label => {
        const t = TIERS.find(x => x.label === label);
    ...
    // ── Sensitivity table: cleared-group n/ROI at each threshold, pooled per tier ──
    const sensitivity = CALIB_TIER_LABELS.map(label => {
      const t = TIERS.find(x => x.label === label);
      const inTier = holdoutMatched.filter(r => r.modelProb >= t.lo && r.modelProb < t.hi);
      const row = { tier: label };
      for (const th of THRESHOLDS) {
        row[`edge_ge_${Math.round(th * 100)}pct`] = statsFor(inTier.filter(r => r.edge >= th));
      }
      return row;
    });
