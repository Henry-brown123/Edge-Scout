...
// TEMPORARY DIAGNOSTIC — per-tier disaggregation of the Platt-scaling ROI
// result from tier-calibration-analysis.md Addendum 2. NOT a new fit and NOT
// a new rule-3 "look" — reuses the exact already-chosen pooled parameters
// (A=1.83126, B=0.15854, decided and reported in Addendum 2) against the same
// test-set population, just split by tier instead of pooled. Read-only, zero
// Odds API calls. To be removed once the doc extension is written.
app.get('/api/debug/platt-roi-by-tier', (_req, res) => {
  ...
  const VALIDATED_LEAGUES = [39, 61, 2, 135];
  const BAND_MIN = 0.45, BAND_MAX = 0.70;
  // Fixed, already-chosen pooled Platt parameters from Addendum 2 — not refit here.
  const PLATT_A = 1.83126, PLATT_B = 0.15854;
  ...
  const correctedOf = raw => sigmoid(PLATT_B + PLATT_A * logit(raw));
  ...
  // Rebuild the identical test-set, band-scoped, Pinnacle-matched population used in Addendum 2.
  const roiRows = [];
  for (const rec of scoredRecords) {
    ...
    if (modelProb < BAND_MIN || modelProb >= BAND_MAX) continue;
    const implied = 1 / odds;
    const won = rec.actualOutcome === topOutcome;
    const correctedProb = correctedOf(modelProb);
    const edgeBefore = (modelProb - implied) / implied;
    const edgeAfter  = (correctedProb - implied) / implied;
    roiRows.push({ leagueId, league: LEAGUE_CONFIG[leagueId].name, tier: tierOf(modelProb), modelProb, correctedProb, implied, odds, won, edgeBefore, edgeAfter });
  }

  const PROPORTIONAL_FLOOR = Math.round(300 / 5); // rule-6's ~300-400 floor, scaled down for 5 tiers ≈ 60

  const byTier = TIER_EDGES.slice(0, -1).map((e, i) => {
    const label = `${Math.round(e*100)}-${Math.round(TIER_EDGES[i+1]*100)}%`;
    const rows = roiRows.filter(r => r.tier === label);
    const beforeQualified = rows.filter(r => r.edgeBefore >= 0.05);
    const afterQualified  = rows.filter(r => r.edgeAfter  >= 0.05);
    const beforeReturns = beforeQualified.map(r => r.won ? (r.odds - 1) : -1);
    const afterReturns  = afterQualified.map(r => r.won ? (r.odds - 1) : -1);

    const alreadyQualified = rows.filter(r => r.edgeBefore >= 0.05 && r.edgeAfter >= 0.05);
    const newlyQualified   = rows.filter(r => r.edgeBefore <  0.05 && r.edgeAfter >= 0.05);
    const droppedOut       = rows.filter(r => r.edgeBefore >= 0.05 && r.edgeAfter <  0.05);
    const alreadyReturns = alreadyQualified.map(r => r.won ? (r.odds - 1) : -1);
    const newlyReturns   = newlyQualified.map(r => r.won ? (r.odds - 1) : -1);

    return {
      tier: label,
      totalMatched: rows.length,
      before: { posEdgeN: beforeQualified.length, belowFloor: beforeQualified.length < PROPORTIONAL_FLOOR, ...ciFor(beforeReturns) },
      after:  { posEdgeN: afterQualified.length,  belowFloor: afterQualified.length  < PROPORTIONAL_FLOOR, ...ciFor(afterReturns) },
      alreadyQualified: { n: alreadyQualified.length, ...ciFor(alreadyReturns) },
      newlyQualified:   { n: newlyQualified.length, ...ciFor(newlyReturns) },
      droppedOutN: droppedOut.length,
    };
  });

  res.json({
    scope: { validatedLeagues: VALIDATED_LEAGUES.map(l => LEAGUE_CONFIG[l].name), band: [BAND_MIN, BAND_MAX], plattParams: { A: PLATT_A, B: PLATT_B }, proportionalFloor: PROPORTIONAL_FLOOR },
    byTier,
  });
});
