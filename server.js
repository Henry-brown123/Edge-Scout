...
// ─── TEMP DIAGNOSTIC (2026-09-01, round 5): does the FIFA-rank blend help or hurt ───
// under GBDT? (server.js scoreOneFixture ~1435-1459: for club_european/international
// context when dataConf<1, blends GBDT's probs toward a FIFA-rank-based anchor —
// disabled for club_domestic, rankScale=0 there.) No closing-odds dependency needed
// here (unlike the edge-based checks), so this uses the full scoredRecords population,
// not just the closing-odds-matched subset. Multi-class Brier score (sum of squared
// error across all 3 outcome probabilities vs the one-hot actual result) compares
// blended vs unblended, restricted to the actual affected population (dataConf<1).
// Simplification: skips the neutral-venue (WC group/knockout) 0.34/0.34 home-base
// override for tractability — uses cfg.homeBase/awayBase uniformly. Remove once answered.
async function computeRankBlendOverlayCheck() {
  ...
  const byContext = { club_european: [], international: [] };
  let sinceYield = 0;
  for (const rec of scoredRecords) {
    ...
    if (context !== 'club_european' && context !== 'international') continue;
    ...
    if (dataConf >= 1) continue; // blend never fires here — not part of the affected population

    const cfg = CONTEXT_CONFIG[context];
    const homeRank = lookupFIFARank(rec.homeTeamName);
    const awayRank = lookupFIFARank(rec.awayTeamName);
    const rankDiff = rankToQuality(homeRank) - rankToQuality(awayRank);
    ...
    const actual = { home: rec.actualOutcome === 'home' ? 1 : 0, draw: rec.actualOutcome === 'draw' ? 1 : 0, away: rec.actualOutcome === 'away' ? 1 : 0 };
    const brier3 = p => (p.home - actual.home) ** 2 + (p.draw - actual.draw) ** 2 + (p.away - actual.away) ** 2;
    byContext[context].push({ unblended: brier3(probs), blended: brier3(blended), dataConf });
  }
  ...
  function summarise(rows) {
    ...
    return {
      n,
      unblendedBrier: +(rows.reduce((s, r) => s + r.unblended, 0) / n).toFixed(4),
      blendedBrier:   +(rows.reduce((s, r) => s + r.blended, 0) / n).toFixed(4),
    };
  }

  return { clubEuropean: summarise(byContext.club_european), international: summarise(byContext.international) };
}
app.get('/api/admin/rankblend-overlay-check', async (_req, res) => {
  try { res.json(await computeRankBlendOverlayCheck()); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
