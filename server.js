...
// TEMP diagnostic — re-run the original-9-leagues 45-70% calibration breakdown
// (Addendum 2/Extension 3's methodology: tier x pick-type, mean predicted vs
// hit rate) against the now-current matched-odds population (post Addendum 26
// staleness fix), for direct comparison against Addendum 23's synthesis.
// Read-only, no writes. Test-only (post each league's own VALIDATED_SPLITS
// testFrom), matching Addendum 2/Extension 3's own rule-1 discipline, so the
// comparison is apples-to-apples. Removed once this diagnostic concludes.
app.get('/api/admin/original9-calibration-refresh', async (_req, res) => {
  try {
    const matched = await computeMatchedEdgeFixtures();
    const ORIGINAL_9 = new Set([135, 39, 61, 2, 179, 78, 140, 88, 94]);
    const ADDENDUM2_4 = new Set([135, 39, 61, 2]); // Serie A, PL, Ligue 1, CL — Addendum 2's exact scope
    const TIERS = [
    ...
    function calibStats(fixtures) {
      const n = fixtures.length;
      if (n === 0) return { n: 0, meanPred: null, hitRate: null, calibErrPp: null };
      const meanPred = fixtures.reduce((s, f) => s + f.modelProb, 0) / n;
      const hitRate = fixtures.filter(f => f.won).length / n;
      // positive = underconfident (actual > predicted), negative = overconfident — same convention as Addendum 25/26.
      const calibErrPp = parseFloat(((hitRate - meanPred) * 100).toFixed(1));
      return { n, meanPred: parseFloat((meanPred * 100).toFixed(1)), hitRate: parseFloat((hitRate * 100).toFixed(1)), calibErrPp };
    }

    const pool9 = matched.filter(f => ORIGINAL_9.has(parseInt(f.leagueId, 10)) && f.modelProb >= 0.45 && f.modelProb < 0.70 && testOnly(f));
    const pool4 = pool9.filter(f => ADDENDUM2_4.has(parseInt(f.leagueId, 10)));

    const tierBreakdown9 = TIERS.map(t => ({ tier: t.label, ...calibStats(pool9.filter(f => f.modelProb >= t.min && f.modelProb < t.max)) }));
    const tierBreakdown4_addendum2Scope = TIERS.map(t => ({ tier: t.label, ...calibStats(pool4.filter(f => f.modelProb >= t.min && f.modelProb < t.max)) }));

    const pickTypeByTier9 = TIERS.map(t => {
      const inTier = pool9.filter(f => f.modelProb >= t.min && f.modelProb < t.max);
      return {
        tier: t.label,
        home: calibStats(inTier.filter(f => f.topOutcome === 'home')),
        away: calibStats(inTier.filter(f => f.topOutcome === 'away')),
        draw: calibStats(inTier.filter(f => f.topOutcome === 'draw')),
      };
    });

    const pickTypePooled9 = {
      home: calibStats(pool9.filter(f => f.topOutcome === 'home')),
      away: calibStats(pool9.filter(f => f.topOutcome === 'away')),
      draw: calibStats(pool9.filter(f => f.topOutcome === 'draw')),
    };

    // Direct Extension 3 comparison: 50-55% tier only, pick-type split.
    const tier5055 = pool9.filter(f => f.modelProb >= 0.50 && f.modelProb < 0.55);
    const ext3Comparison5055 = {
      home: calibStats(tier5055.filter(f => f.topOutcome === 'home')),
      away: calibStats(tier5055.filter(f => f.topOutcome === 'away')),
      draw: calibStats(tier5055.filter(f => f.topOutcome === 'draw')),
    };

    // Per-league matched-data date range within this population, for freshness transparency.
    const leagueDateRanges = {};
    for (const lid of ORIGINAL_9) {
      const rows = matched.filter(f => parseInt(f.leagueId, 10) === lid);
      if (!rows.length) { leagueDateRanges[lid] = null; continue; }
      const dates = rows.map(f => f.date).sort();
      leagueDateRanges[lid] = { n: rows.length, from: dates[0], to: dates[dates.length - 1] };
    }

    res.json({
      scope: 'original 9 leagues, 45-70% raw modelProb band, test-only (post each league\'s own VALIDATED_SPLITS testFrom), current matched-odds data',
      pool9N: pool9.length,
      pool4N: pool4.length,
      tierBreakdown9,
      tierBreakdown4_addendum2Scope,
      pickTypePooled9,
      pickTypeByTier9,
      ext3Comparison5055,
      leagueDateRanges,
