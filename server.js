...
// TEMPORARY — the single, deliberate calibration + ROI look for Carabao Cup/
// League One/League Two (task: "Closing-odds backfill and single, deliberate
// calibration + ROI read"). Deliberately NOT runEvCalibration() — that function
// also auto-manages settings.paperTradeOnly (add/remove based on ROI) and the
// paper Kelly fraction, side effects designed for the 9 validated leagues'
// ongoing weekly cron cycle. This population is categorically different (fully
// unseen, no train/test split, one-time baseline per calibration-rules.md rule
// 10) and any paper/real decision from it should stay a deliberate human call,
// not something this read triggers automatically. This endpoint is read-only —
// it computes and returns, it does not write to settings.json or any other file.
//
// Methodology mirrors runEvCalibration() exactly for the model-probability/edge
// computation (GBDT model.predict -> applyLeagueBiasCorrection, matched against
// Pinnacle closing odds), then buckets by predicted-probability tier (5pp,
// TIER_EDGES_SHARED/tierOfProbShared — the same tiers LEAGUE_TIER_MATRIX uses)
// rather than runEvCalibration()'s edge-band scheme. Calibration cells use every
// matched fixture in a tier (predicted vs actual hit rate); ROI cells use only
// the posEdge>=5% subset within that tier, matching LEAGUE_TIER_MATRIX's
// convention. CI is a normal approximation on the sample mean of individual bet
// returns (mean ± 1.96*SE) — standard for a bounded-outcome sample mean, no
// hand-picked parameters. Shrinkage pools among these 3 leagues' own cells only,
// deliberately not merged into the original 9-league LEAGUE_TIER_MATRIX pool —
// keeps every already-published Addendum 6 number completely untouched.
app.get('/api/debug/single-look-3leagues', async (req, res) => {
  try {
    const { empiricalBayesShrink, varianceForRoi, varianceForHitRate } = require('./shrinkage');
    const { classifyFixture, applyLeagueBiasCorrection, LEAGUE_CONFIG } = require('./scoring');

    const NEW_LEAGUE_IDS = [48, 41, 42];
    const NEW_LEAGUE_NAMES = { 48: 'Carabao Cup', 41: 'League One', 42: 'League Two' };
    ...
    // ── Build the matched population (mirrors runEvCalibration exactly) ──────
    const matched = [];
    for (const rec of scoredRecords) {
      const lid = parseInt(rec.leagueId, 10);
      if (!NEW_LEAGUE_IDS.includes(lid)) continue;
      const co = closingOdds[rec.fixtureId] || closingOdds[String(rec.fixtureId)];
      if (!co) continue;
      if (!rec.actualOutcome) continue;
      if (!rec.homeFactors || !rec.awayFactors) continue;
      ...
      const edge = (modelProb - pinnacleImplied) / pinnacleImplied;
      const won  = rec.actualOutcome === topOutcome;
      const isHomePick = topOutcome === 'home';
      const isAwayPick = topOutcome === 'away';

      matched.push({
        fixtureId: rec.fixtureId, leagueId: lid, leagueName: NEW_LEAGUE_NAMES[lid],
        topOutcome, modelProb, pinnacleOdds, pinnacleImplied, edge, won, date: rec.date,
        isHomePick, isAwayPick,
      });
    }

    const TIERS = TIER_LABELS_SHARED; // reuse the shared tier scheme
    function tierOf(p) { return tierOfProbShared(p); }
    ...
    function roiStats(fixtures) {
      ...
      return {
        n, roi: +mean.toFixed(4),
        ciLow: +(mean - 1.96 * se).toFixed(4), ciHigh: +(mean + 1.96 * se).toFixed(4),
        sampleVariance: variance,
      };
    }
    function calibrationStats(fixtures) {
      const n = fixtures.length;
      if (n === 0) return { n: 0, meanPredicted: null, actualHitRate: null, errorPp: null };
      const meanPredicted = fixtures.reduce((s, f) => s + f.modelProb, 0) / n;
      const hits = fixtures.filter(f => f.won).length;
      const actualHitRate = hits / n;
      return { n, meanPredicted: +meanPredicted.toFixed(4), actualHitRate: +actualHitRate.toFixed(4), errorPp: +((meanPredicted - actualHitRate) * 100).toFixed(1) };
    }

    // ── Pooled (3 leagues combined) tier grids ────────────────────────────
    const pooledCalibration = TIERS.map(tier => {
      const inTier = matched.filter(f => tierOf(f.modelProb) === tier);
      return { tier, ...calibrationStats(inTier) };
    }).filter(t => t.n > 0);

    const pooledRoi = TIERS.map(tier => {
      const inTier = matched.filter(f => tierOf(f.modelProb) === tier);
    ...
    const perLeagueRoiCells = [];
    const perLeagueCalibCells = [];
    for (const lid of NEW_LEAGUE_IDS) {
      for (const tier of TIERS) {
        const inTier = matched.filter(f => f.leagueId === lid && tierOf(f.modelProb) === tier);
        if (inTier.length === 0) continue;
        const posEdge = inTier.filter(f => f.edge >= 0.05);
        const rs = roiStats(posEdge);
        if (posEdge.length > 0) perLeagueRoiCells.push({ id: `${lid}_${tier}`, leagueId: lid, tier, n: rs.n, value: rs.roi, sampleVariance: rs.sampleVariance, ciLow: rs.ciLow, ciHigh: rs.ciHigh, totalN: inTier.length });
        const cs = calibrationStats(inTier);
        perLeagueCalibCells.push({ id: `${lid}_${tier}`, leagueId: lid, tier, n: cs.n, value: cs.actualHitRate, meanPredicted: cs.meanPredicted, errorPp: cs.errorPp });
      }
    }
    const shrunkRoiCells    = empiricalBayesShrink(perLeagueRoiCells, varianceForRoi);
    const shrunkCalibCells  = empiricalBayesShrink(perLeagueCalibCells.map(c => ({ ...c, value: c.value ?? 0 })), varianceForHitRate);

    const byLeagueRoi = NEW_LEAGUE_IDS.map(lid => ({
      leagueId: lid, leagueName: NEW_LEAGUE_NAMES[lid],
      cells: shrunkRoiCells.filter(c => c.leagueId === lid).map(c => ({
        tier: c.tier, n: c.n, totalN: c.totalN, roi: c.value, ciLow: c.ciLow, ciHigh: c.ciHigh,
        shrunk: c.shrunk, weight: c.weight, thin: c.n < 30,
      })),
    }));
    const byLeagueCalibration = NEW_LEAGUE_IDS.map(lid => ({
      leagueId: lid, leagueName: NEW_LEAGUE_NAMES[lid],
      cells: shrunkCalibCells.filter(c => c.leagueId === lid).map(c => ({
        tier: c.tier, n: c.n, meanPredicted: c.meanPredicted, actualHitRate: c.value, errorPp: c.errorPp,
        shrunkHitRate: c.shrunk, shrunkErrorPp: c.meanPredicted != null ? +((c.meanPredicted - c.shrunk) * 100).toFixed(1) : null,
      })),
    }));

    // ── Home/away split — pooled and per-league where sample allows ──────
    function homeAwaySplit(fixtures) {
      const home = fixtures.filter(f => f.isHomePick);
    ...
    const byLeagueHomeAway = NEW_LEAGUE_IDS.map(lid => ({
      leagueId: lid, leagueName: NEW_LEAGUE_NAMES[lid],
      ...homeAwaySplit(matched.filter(f => f.leagueId === lid)),
    }));
    ...
    res.json({
      note: 'SINGLE LOOK — calibration-rules.md rule 3. Do not re-run this endpoint with different logic/parameters and expect a fresh look; this population is now considered examined.',
      matchedPopulation: { total: matched.length, byLeague: byLeagueOverall },
      overall,
      calibration: { pooled: pooledCalibration, byLeague: byLeagueCalibration },
      roi: { pooled: pooledRoi, byLeague: byLeagueRoi },
      homeAway: { pooled: pooledHomeAway, byLeague: byLeagueHomeAway },
    });
