...
// TEMP DIAGNOSTIC — EL/Conference League split planning. Exposes the raw
// matched-Pinnacle population (same matching logic as runEvCalibration) for
// leagues 3/848, chronologically sorted, with actualOutcome attached so a
// genuine time-based split boundary and train-only base-rate frequencies can
// be computed per calibration-rules.md. Remove once the split is committed.
app.get('/api/diagnostics/el-conf-split-planning', (_req, res) => {
  const historical  = readJSON('backfill-historical.json') || {};
  const scoredRecords = historical.scoredRecords || [];
  const closingOdds = readJSON('closing-odds.json') || {};
  const optWeights  = historical.optimisedWeights || {};
  ...
  const TARGET_LEAGUES = [3, 848];
  const byLeague = {};

  for (const rec of scoredRecords) {
    const lid = parseInt(rec.leagueId, 10);
    if (!TARGET_LEAGUES.includes(lid)) continue;
    const co = closingOdds[rec.fixtureId] || closingOdds[String(rec.fixtureId)];
    if (!co) continue;
    if (!rec.actualOutcome || !rec.homeFactors || !rec.awayFactors) continue;
    ...
    byLeague[lid].push({
      fixtureId: rec.fixtureId, date: rec.date, actualOutcome: rec.actualOutcome,
      topOutcome, modelProb, pinnacleOdds, edge, won,
    });
  }

  const result = {};
  for (const [lid, fxs] of Object.entries(byLeague)) {
    fxs.sort((a, b) => new Date(a.date) - new Date(b.date));
    const home = fxs.filter(f => f.actualOutcome === 'home').length;
    const draw = fxs.filter(f => f.actualOutcome === 'draw').length;
    const away = fxs.filter(f => f.actualOutcome === 'away').length;
    result[lid] = {
      leagueName: LEAGUE_CONFIG[lid]?.name,
      n: fxs.length,
      dateRange: fxs.length ? [fxs[0].date, fxs[fxs.length - 1].date] : null,
      overallOutcomeRate: fxs.length ? { home: +(home/fxs.length).toFixed(4), draw: +(draw/fxs.length).toFixed(4), away: +(away/fxs.length).toFixed(4) } : null,
      // Year-month histogram to see actual coverage density/gaps (e.g. the
      // Conference League 2021-22 blackout confirmed in Addendum 11).
      monthHistogram: fxs.reduce((acc, f) => {
