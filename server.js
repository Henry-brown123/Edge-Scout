...
// TEMPORARY diagnostic endpoint — Serie A train/test split analysis for the
// disciplined calibration cycle (docs/calibration-rules.md). Read-only. Builds the
// same matched population as runEvCalibration(), sorted chronologically, splits at
// a configurable fraction, and reports train-only observed rates + both sides'
// ROI under the CURRENT (untouched) LEAGUE_CONFIG — no tuning happens here, this is
// pure diagnostics to inform a train-only decision. Remove after Part B is done.
app.get('/api/debug/serie-a-split', (req, res) => {
  try {
    ...
    for (const rec of scoredRecords) {
      if (parseInt(rec.leagueId, 10) !== SERIE_A) continue;
    ...
    function evalSet(records, leagueConfigOverride) {
      ...
      for (const rec of records) {
        ...
        const rawM = computeModelProb(rec.homeFactors, rec.awayFactors, rec.weights, rec.context, cfg[SERIE_A]);
        modelHomeSum += rawM.home; modelDrawSum += rawM.draw; modelAwaySum += rawM.away;

        const rawProbs = model.predict(rec.homeFactors, rec.awayFactors, rec.weights, rec.context, cfg[SERIE_A]);
        const probs    = applyLeagueBiasCorrection(rawProbs, SERIE_A, cfg);
    ...
    res.json({
      ...
      currentConfig: LEAGUE_CONFIG[SERIE_A],
