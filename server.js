...
// TEMP — verification task only, remove after findings delivered. PARTIAL
// reconstruction of live successScore gating against the historical population for
// League One (41) / League Two (42), the 6 currently green-flagged cells. NOT a
// faithful reproduction of live behavior — deliberately omits weather/WOWY/team-
// profile modifiers (applyTeamProfileModifiers never runs in the historical scorer;
// server.js:4045 already documents this as a known, accepted gap) since there is no
// historical record of those inputs to reconstruct from, and fabricating them would
// violate calibration-rules.md. Reconstructs: calFactor boost, margin-stripped
// Pinnacle normalization, absolute (not relative) edge, and dataConf/formFixtureCount
// from the historical team-fixture index. Treat the resulting overlap % as a LOWER
// BOUND on the true mismatch, not a measurement of it.
app.get('/api/diagnostics/live-gate-overlap', (req, res) => {
  ...
  const settings = getSettings();
  const calFactor = settings.calibrationFactor ?? 1.08;
  const successThreshold = settings.successThreshold || 40;
  if (!scoredRecords.length) return res.json({ error: 'No historical data' });

  const teamIndex = buildTeamIndex(fixtures);
  const fixtureById = new Map(fixtures.map(f => [f.fixture?.id, f]));

  const TARGET_LEAGUE_IDS = new Set([41, 42]);
  const rows = [];
  ...
  for (const rec of scoredRecords) {
    ...
    if (!pinnacleOdds || pinnacleOdds <= 1 || !co.homeOdds || !co.awayOdds || !co.drawOdds) continue;

    // OLD test — exactly runEvCalibration/walkforward's methodology: relative edge vs
    // raw (non-margin-stripped) Pinnacle implied probability, tier from raw modelProb.
    const pinnacleImplied = 1 / pinnacleOdds;
    const oldEdge = (modelProb - pinnacleImplied) / pinnacleImplied;
    const oldPass = oldEdge >= 0.05;
    const tier = tierOfProbShared(modelProb);

    // NEW test — reconstructed live successScore.
    const calProb = Math.min(0.97, modelProb * calFactor);
    const rawImplied = { home: 1 / co.homeOdds, away: 1 / co.awayOdds, draw: 1 / co.drawOdds };
    const sumImplied = rawImplied.home + rawImplied.away + rawImplied.draw;
    const strippedImplied = rawImplied[topOutcome] / sumImplied;
    const pinnacleEdgeVsMarket = calProb - strippedImplied;

    const fix = fixtureById.get(rec.fixtureId);
    const homeId = fix?.teams?.home?.id, awayId = fix?.teams?.away?.id;
    const fixDate = fix?.fixture?.date ? new Date(fix.fixture.date) : null;
    const countPrior = (teamId) => {
      if (!teamId || !fixDate) return 0;
      const hist = teamIndex[teamId] || [];
      return hist.filter(f => new Date(f.fixture?.date) < fixDate).length;
    };
    const homeFormCount = countPrior(homeId);
    const awayFormCount = countPrior(awayId);
    const confCap = context === 'international' ? 0.70 : 1;
    const homeDataConf = Math.min(homeFormCount / 15, confCap);
    const awayDataConf = Math.min(awayFormCount / 15, confCap);
    const dataConf = Math.min(homeDataConf, awayDataConf);

    // displayOdds — live uses a separate "UK book odds" price; only Pinnacle is
    // available historically, so raw Pinnacle closing odds are used as the closest
    // available proxy (same limitation applies uniformly across the whole population).
    const rawScore = computeSuccessScore(calProb, pinnacleOdds, homeFormCount, dataConf, pinnacleEdgeVsMarket, leagueId, context);
    // wxMod omitted entirely (no historical weather record) — NOT set to a neutral
    // stand-in value; finalScore below is the score BEFORE any weather adjustment,
    // whatever direction that would have pushed it.
    const effMult = 1 / (LEAGUE_CONFIG[leagueId]?.marketEfficiency ?? 1.0);
    const finalScore = Math.round(rawScore * effMult);
    const newPass = finalScore >= successThreshold;

    rows.push({ leagueId, tier, oldPass, newPass, finalScore, oldEdge: +oldEdge.toFixed(4) });
  }

  function summarize(subset) {
    const n = subset.length;
    const oldPassN = subset.filter(r => r.oldPass).length;
    const newPassN = subset.filter(r => r.newPass).length;
    const bothN = subset.filter(r => r.oldPass && r.newPass).length;
    const oldOnlyN = subset.filter(r => r.oldPass && !r.newPass).length;
    const newOnlyN = subset.filter(r => !r.oldPass && r.newPass).length;
    return {
      n, oldPassN, newPassN,
      bothN, oldOnlyN, newOnlyN,
      pctOldThatAlsoClearNew: oldPassN ? +((bothN / oldPassN) * 100).toFixed(1) : null,
      pctNewThatOldMissed: newPassN ? +((newOnlyN / newPassN) * 100).toFixed(1) : null,
    };
  }

  const GREEN_FLAG_CELLS = [
    { leagueId: 42, tier: '45-50%' }, { leagueId: 41, tier: '45-50%' },
    { leagueId: 42, tier: '50-55%' }, { leagueId: 42, tier: '55-60%' },
    { leagueId: 42, tier: '65-70%' }, { leagueId: 41, tier: '60-65%' },
  ];

  res.json({
    overall41: summarize(rows.filter(r => r.leagueId === 41)),
    overall42: summarize(rows.filter(r => r.leagueId === 42)),
    greenFlaggedCells: GREEN_FLAG_CELLS.map(c => ({
      ...c,
      ...summarize(rows.filter(r => r.leagueId === c.leagueId && r.tier === c.tier)),
    })),
  });
});
